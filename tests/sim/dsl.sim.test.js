...
test(
  "DSL sim: dslDiff unbiased with honest coverage while naive diff is biased",
  { timeout: 120000 },
  () => {
    let sumDsl = 0;
    let sumNaive = 0;
    ...
    for (let rep = 0; rep < REPS; rep++) {
      ...
      const dslRows = units.map((u) =>
        u.pi !== undefined ? { yhat: u.yhat, y: u.y, pi: u.pi, x: u.x } : { yhat: u.yhat, x: u.x },
      );
    ...
    assert.ok(olsCoverage >= 0.91 && olsCoverage <= 0.985, `dslOLS slope coverage ${olsCoverage}`);
  },
);
...
test(
  "R1 sim: auto-λ variance ≤ classical and ≤ gold-only (+MC slack); honest coverage",
  { timeout: 120000 },
  () => {
    const N = 1000;
    const NG = 100;
...
test(
  "V1 sim (high gold fraction): auto-λ coverage stays honest at n_g/n = 1/2",
  { timeout: 120000 },
  () => {
    const N2 = 600;
    const NG2 = 300;
