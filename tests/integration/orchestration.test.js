...
test("silverTune: real engine + real stability — loop runs, curve recorded, promotion fires, goldset persists with pi, ledger in order", async () => {
  ...
  assert.deepEqual(
    instrument.payload.schema,
    { type: "binary", options: ["yes", "no"] },
  ...
  const {
    ...
    curve,
    cost,
  } = await silverTune(project, instrument, units, {
  ...
  const gs = JSON.parse(
    await readFile(path.join(projectDir(slug), "gold", `${gsMeta.id}.json`), "utf8"),
  );
  assert.equal(gs.status, "complete");
  assert.equal(gs.constructId, construct.id);
  assert.equal(gs.sample.length, 200);
  assert.ok(
    gs.sample.every((s) => s.pi === 200 / 300),
    "pi = n/N recorded on every sample row",
  );
  ...
  assert.equal(
    curve[0].agreement,
    planted,
    "iteration-1 agreement is exactly the planted disagreement rate",
  );
  ...
  assert.match(
    rewritePromptSeen,
    /EDGEC/,
    "the rewrite prompt carried real confused-unit evidence",
  );
  assert.notEqual(
    curve[0].versionHash,
    curve[1].versionHash,
    "each iteration ran a distinct instrument version",
  );
  assert.equal(tuned.versionHash, curve[1].versionHash);
  assert.equal(tuned.version, 2);
  assert.match(
    tuned.payload.promptTemplate,
    /Revised v1/,
    "the Director's rewrite is the final template",
  );
  ...
  assert.ok(
    existsSync(path.join(projectDir(slug), "cache")),
    "runEphemeral wrote the bundle cache (proof the real engine served the loop)",
  );
  ...
  assert.deepEqual(
    cost,
    { workerUSD: 0, directorUSD: 0 },
    "silverTune returns cost: {workerUSD, directorUSD}",
  );
...
test("BUG-1 regression: one stability check ledgers exactly ONE instrument.stability event (stability.js owns the append)", async () => {
  ...
  const events = await ledger.query(silverState.pdir, {
    type: "instrument.stability",
    ref: silverState.instrumentId,
  });
  assert.equal(
    events.length,
    1,
...
test("silverTune: a tiny {capUSD} stops cleanly after iteration 1 — stoppedBy budget, curve note, no rewrite paid for, partial tune valid", async (t) => {
  ...
  const { project, units } = await makeProject({
    ...
    n: 30,
    textOf: (i) =>
      `Response ${i}: ${i % 3 === 0 ? "the salary is low" : "the chairs are fine"}`.padEnd(60, "."),
  });
  const byId = new Map(units.map((u) => [u.id, u]));
  mock.setAccuracy(1.0);
  mock.setOracle(ORACLE);
  ...
  assert.equal(
    rewrites,
    0,
    "the stop preempts the next iteration's Director rewrite — nothing paid for it",
  );
  assert.match(res.curve[0].note, /budget/i, "the budget stop is recorded on the curve");
  assert.ok(
    res.cost.workerUSD >= 1e-6,
    `accumulated worker spend crossed the cap (got ${res.cost.workerUSD})`,
  );
...
function patchWorkerCalls(t, fn) {
  ...
  }); // fall back to the prototype method
}
...
test("silverTune: a PROVIDER_REFUSAL inside the real engine quarantines that unit; the loop continues over the compared rest", async (t) => {
  ...
  const gs = JSON.parse(
    await readFile(path.join(projectDir(slug), "gold", `${tuned.silver.goldsetId}.json`), "utf8"),
  );
  assert.equal(Object.keys(gs.coders[0].labels).length, 12);
  assert.equal(
    curve[0].agreement,
    1,
    "agreement computed over the 11 compared (non-refused) units",
  );
...
test("silverTune: PROVIDER_UNREACHABLE inside the real engine propagates after Pool retries; goldset survives, instrument is NOT silver_tuned", async (t) => {
  ...
  patchWorkerCalls(t, () => {
    throw new ConcordError("PROVIDER_UNREACHABLE", "connect ECONNREFUSED (injected)", {
      kind: "TypeError",
    });
  });
  ...
  await assert.rejects(
    ...
    (err) => {
      assert.equal(
        ...
        "infrastructure faults keep their identity across the seam",
      );
      return true;
    },
  );
  ...
  const pdir = projectDir(slug);
  assert.equal(
    (await ledger.query(pdir, { type: "goldset.created" })).length,
    1,
    "silver labels are not lost to a worker outage",
  );
...
test("unit wrapper: assembled worker messages carry exactly ONE <unit>/</unit> pair — scaffolded slot and legacy templates alike", async (t) => {
  ...
  assert.ok(
    scaffolded.payload.promptTemplate.includes("{{unit}}"),
    "scaffolding appended the unit slot",
  );
  assert.ok(
    !scaffolded.payload.promptTemplate.includes("<unit>"),
...
test("pipeline: compileInstrument (mock director) → createRun → executeRun + makeEscalator → monitor sane → suggestAnalyses materializable", async () => {
  ...
  const { project, corpusId, units } = await makeProject({
    ...
    textOf: (i) =>
      i === LONG
        ? `Response ${i}: ` +
          "the salary conversation keeps coming back and nobody addresses it properly. ".repeat(10)
        : (
            `Response ${String(i).padStart(3, "0")}: ` +
  ...
  mock.setHandler("dir-pipe", (req) => {
    ...
    if (req.schema?.properties?.reason) {
      ...
      return {
        rationale:
          "The unit is repetitive boilerplate; no concrete compensation complaint is made.",
        label: "no",
        confidence: 0.95,
    ...
    throw new Error(
      `dir-pipe handler got an unexpected Director request: ${JSON.stringify(req.schema?.properties ? Object.keys(req.schema.properties) : null)}`,
    );
  });
  ...
  for (const slot of ["{{definition}}", "{{criteria}}", "{{examples}}", "{{unit}}"]) {
    assert.ok(
      instrument.payload.promptTemplate.includes(slot),
      `compiled template carries ${slot}`,
    );
  }
  await acceptInstrument(project, instrument);
  Object.assign(project, await loadProject(slug));
  ...
  assert.match(
    escalated[0].rationale,
    /^Worker over-weighted .* — /,
    "reason-first rationale from makeEscalator",
  );
  ...
  assert.equal(
    ...
    "the worker's juror hash stays on the line (resume semantics)",
  );
  assert.equal(
    escalated[0].escalatedBy,
    "director",
    "the Director's provenance marker lands on the written line",
  );
  assert.ok(
    lines.filter((l) => !l.escalated).every((l) => l.escalatedBy === undefined),
  ...
  assert.deepEqual(
    lastTick.labelDist,
    { yes: 100, no: 200 },
    "planted distribution, with the escalated unit flipped to no",
  );
  assert.deepEqual(
    lastTick.warnings,
    [],
    "no degenerate/drift/quarantine warnings on a healthy run",
  );
...
test("brief: generateBrief over the real corpus — refs validated, invalid refs counted, artifact + ledger persist, paragraphs stream in order", async () => {
  ...
  mock.setHandler("dir-brief", (req) => {
    ...
    return {
      ...
      themes: [
        {
          name: "Pay",
          definition: "Complaints about compensation level.",
          quoteRefs: [ids[0], ids[3], ids[4]],
        },
      ],
      redFlags: [
        {
          kind: "junk",
          detail: "One unit looks like filler.",
          refs: ["u_beefbeefbeefbeef", ids[5]],
        }, // 1 invalid + 1 valid
      ],
  ...
  assert.deepEqual(
    streamed.map((s) => s.i),
    [0, 1],
    "paragraphs streamed in order for SSE relay",
  );

  // persistence: artifact file + project registry + ledger
  const onDisk = JSON.parse(
    await readFile(path.join(projectDir(slug), "briefs", `${brief.id}.json`), "utf8"),
  );
...
test("questionbar: compileQuestion → approvePlan materializes against REAL judge.outputSchemaFor → createRun preflight estimate present", async (t) => {
  ...
  const { project, corpusId, units } = await makeProject({
    ...
    n: 60,
    textOf: (i) =>
      `Response ${i}: ${i % 3 === 0 ? "the salary is too low" : "the team is kind"}`.padEnd(
        70,
        ".",
      ),
  });
  ...
  mock.setHandler("dir-qbar", (req) => {
    if (req.schema?.properties?.promptTemplate) {
      return {
        promptTemplate: "Plan judge. {{definition}} {{criteria}} {{examples}} {{unit}}",
        note: "compiled for approval",
      };
    }
    return {
      constructs: [
...
test("concurrency: two concurrent executeRuns (judge pool-4 + dictionary) keep monitor states, outputs and the ledger chain intact", async () => {
  ...
  const { project, corpusId, units } = await makeProject({
    ...
    n: 150,
    textOf: (i) =>
      `Response ${i}: ${i % 3 === 0 ? "the salary is low" : "the chairs are fine"}`.padEnd(70, "."),
  });
  mock.setAccuracy(1.0);
  mock.setOracle(ORACLE);
  ...
  const [d1, d2] = await Promise.all([
    executeRun(slug, r1.id, {
      onTick: (s) => {
        s1 = s;
      },
    }),
    executeRun(slug, r2.id, {
      onTick: (s) => {
        s2 = s;
      },
    }),
  ...
  assert.deepEqual(
    s2.labelDist,
    { yes: 50, no: 100 },
    "dictionary instrument agrees with the planted theme",
  );
...
test("BUG-2 regression: the drift re-judge uses the run's bundle dir (armDriftTripwire {dir} → driftTick → runEphemeral), not the default projects dir", async (t) => {
  ...
  const run = await createRun(
    project,
    { instrumentId: "inst_drift", corpusId: "c1" },
    { dir: bundleDir },
  );
  ...
  const cacheFiles = (await readdir(bundleCache, { recursive: true, withFileTypes: true })).filter(
    (e) => e.isFile(),
  );
  assert.ok(
