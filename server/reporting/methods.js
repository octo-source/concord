...
const LEVEL_SENTENCES = {
  ...
  stabilized:
    "Estimates are stabilized for test-retest consistency only; no human validation was performed",
  calibrated:
    "Estimates are calibrated against human gold labels but remain uncorrected for residual machine error",
  corrected:
    "Estimates are corrected for machine misclassification against designed human gold labels",
};
...
export async function loadAnalysis(project, projectDir, analysisId) {
  ...
  const analysis =
    fromFile ?? (project.analyses ?? []).find((a) => a?.id === analysisId && a.spec) ?? null;
  if (!analysis) {
...
async function compose(project, analysisId, { projectDir } = {}, mode) {
  if (!project || typeof project !== "object" || !project.id)
    fail("generate requires a project object");
  ...
  if (analysis.level === "corrected" && !/^(dsl|ppi)/.test(estimator)) {
    fail(
      `analysis '${analysisId}' carries the Corrected level but records no correction estimator`,
      {
        analysisId,
  ...
  const instrument =
    (project.instruments ?? []).find((i) => i.id === (spec.instrumentId ?? run?.instrumentId)) ??
    null;
  const construct =
    (project.constructs ?? []).find(
      (c) => c.id === (instrument?.constructId ?? spec.constructId),
    ) ?? null;
    ...
    canonical({
      ...
      corpusMeta: corpus
        ? {
            ...
            unitization: corpus.unitization ?? null,
      ...
      run: run
        ? {
            ...
            quarantine: run.quarantine ?? null,
            pinned: run.pinned ?? null,
      ...
      goldset: goldset
        ? {
            id: goldset.id,
            design: goldset.design ?? null,
            n: goldset.sample?.length ?? null,
            piSummary: goldPis.length
              ? { min: Math.min(...goldPis), max: Math.max(...goldPis) }
              : null,
            coderIds: (goldset.coders ?? []).map((c) => c.coderId ?? null),
            humanAgreement: goldset.humanAgreement ?? null,
            uncodableUnits: [
              ...new Set((goldset.coders ?? []).flatMap((c) => Object.keys(c.uncodable ?? {}))),
            ].sort(),
      ...
      instrument: instrument
        ? {
            versionHash: instrument.versionHash ?? null,
            frozen: instrument.frozen ?? false,
            certificate: instrument.certificate ?? null,
            stability: instrument.stability ?? null,
  ...
  let anchorEvent;
  if (mode === "export") {
    anchorEvent = await ledger.append(
      projectDir,
      "system",
      "export.methods",
      { projectId: project.id, analysisId },
      { analysisId, stateHash, format: "markdown" },
    );
  } else {
    anchorEvent =
      [...events]
        .reverse()
        .find((e) => e.type === "export.methods" && e.refs?.analysisId === analysisId) ??
  ...
  if (mode === "preview") {
    ...
    out.push(
      s(
        "This section was generated by Concord from the project ledger, and every sentence cites the hash-chained ledger event that substantiates it",
        anchorEvent,
      ),
    );
  } else {
    out.push(
      s(
        "This section was generated by Concord from the project ledger; every sentence cites the hash-chained ledger event that substantiates it, and the export itself is recorded in the same ledger",
        anchorEvent,
      ),
    );
  }
  ...
  section("Data and unitization");
  {
    ...
    const unitCount =
      corpus?.unitCount ??
      unitized?.payload?.unitCount ??
      (corpus
        ? (await readNdjson(path.join(projectDir, "corpora", corpus.id, "units.ndjson"))).length
        : null);
    ...
    if (corpus) {
      sentences.push(
        s(
          `The corpus "${inline(corpus.name ?? corpus.id)}" was imported from ${inline(src.filename ?? "the source file")}${src.format ? ` (${src.format}${rows != null ? `, ${rows} rows` : ""})` : ""}`,
          imported,
        ),
      );
      sentences.push(
        s(
          `Texts were unitized at the ${scheme} level, yielding ${unitCount ?? "the recorded number of"} analyzable units`,
          unitized,
        ),
      );
    } else {
      sentences.push(
        s(
          "No corpus metadata is attached to this analysis; unit counts derive from the stored unit files at export time",
          anchorEvent,
        ),
      );
    }
  ...
  if (construct) {
    ...
    sentences.push(
      s(
        `The construct "${inline(construct.name)}" (${construct.type}) was defined as: "${inline(construct.definition)}"`,
        created,
      ),
    );
    if (construct.authoredBy === "director" && construct.origin === "inductive") {
      ...
      if (construct.humanTouched && edited) {
        sentences.push(
          s(
            `${head}; the codebook entry was subsequently reviewed and edited by the research team`,
            edited,
          ),
        );
      ...
      sentences.push(
        s(
          "The codebook entry was drafted by the AI Director and subsequently reviewed and edited by the research team",
          edited,
        ),
      );
      ...
      sentences.push(
        s(
          "The codebook entry was drafted by the AI Director and subsequently accepted by the research team",
          created,
        ),
      );
      ...
      sentences.push(
        s(
          "The codebook entry was drafted by the AI Director and has not been edited by a human",
          created,
        ),
      );
    } else {
    ...
    sentences.push(
      s(
        `As recorded at export time, the entry specifies ${count(construct.criteria?.include?.length ?? 0, "inclusion criterion", "inclusion criteria")}, ${count(construct.criteria?.exclude?.length ?? 0, "exclusion criterion", "exclusion criteria")}, ${count(construct.edgeCases?.length ?? 0, "documented edge case")} and ${count(construct.examples?.length ?? 0, "worked example")}`,
        anchorEvent,
      ),
    );
    if (instrument?.frozen) {
      sentences.push(
        s(
          `The instrument freeze versions the prompt template, not the codebook entry: these counts read the live construct rather than a snapshot "As frozen for this analysis", which Concord does not yet take`,
          anchorEvent,
        ),
      );
    }
  ...
  if (goldset && goldset.tier === "gold") {
    ...
    sentences.push(
      s(
        queuedN > 0
          ...
          : `A gold-standard validation sample of n = ${nGold} units was drawn as ${design}${strata}`,
        sampled,
      ),
    );
    if (pis.length > 0) {
      ...
      sentences.push(
        s(
          goldset.design === "uncertainty"
            ...
            : `Unit inclusion probabilities were fixed by the design and recorded at sampling time (${piText})`,
          sampled,
        ),
      );
    }
    ...
    if (coders.length > 0) {
      ...
      sentences.push(
        s(
          `${count(coders.length, "human coder")} labeled the sample independently${blind ? "; labels were collected through Concord's blind coding interface, which serves coders no machine labels and no co-coder labels, so each coder was blind to machine labels and to each other's labels" : ""}`,
          labelEv ?? sampled,
        ),
      );
    }
    if (goldset.adjudicated) {
      ...
      sentences.push(
        s(
          "Coder disagreements were resolved by adjudication, and the adjudicated labels, together with units where at least two coders agreed unanimously, constitute the gold standard used below",
          adjudicatedEv ?? anchorEvent,
        ),
      );
    }
    para(...sentences);
  ...
  if (ha) {
    ...
    const MACHINE_COMPARISON_TYPES = new Set([
      "instrument.frozen",
    ...
    const firstMachineIdx = events.findIndex(
      (e) =>
        MACHINE_COMPARISON_TYPES.has(e.type) &&
    ...
    if (typeof ha.kappa === "number")
      bits.push(
        `${construct?.type === "ordinal" ? "linear-weighted Cohen's kappa" : "Cohen's kappa"} = ${fmt(ha.kappa)}`,
      );
    if (typeof ha.alpha === "number")
      bits.push(
        `Krippendorff's alpha = ${fmt(ha.alpha)}${fmtCI(ha.ci) ? `, 95% CI ${fmtCI(ha.ci)} (${ha.ci.method ?? "bootstrap"})` : ""}`,
      );
    const sentences = [
      s(
        orderProven
          ? "Inter-coder reliability was computed before any machine output was compared to the human labels"
          : `Inter-coder reliability was computed from the ${blindCoders ? "blind " : ""}double-coded sample`,
        agreeEv,
      ),
      s(
        `On the ${ha.n ?? goldset?.sample?.length ?? "gold"} jointly coded units the coders reached ${bits.join(", ")}`,
        agreeEv,
      ),
    ];
    ...
    const uncodableN = new Set(
      (goldset?.coders ?? []).flatMap((c) => Object.keys(c.uncodable ?? {})),
    ).size;
    ...
    if (uncodableN > 0 || excludedN > 0) {
      ...
      if (uncodableN > 0)
        clauses.push(
          `${count(uncodableN, "unit")} ${uncodableN === 1 ? "was" : "were"} marked uncodable by at least one coder`,
        );
      if (excludedN > 0)
        clauses.push(
          `${count(excludedN, "unit")} ${excludedN === 1 ? "was" : "were"} excluded from the gold standard after adjudication`,
        );
  ...
  if (instrument) {
    ...
    if (instrument.kind === "judge" || instrument.kind === "panel") {
      ...
      if (instrument.kind === "panel" && paramSets.size > 1) {
        sentences.push(
          s(
            "Decoding parameters varied across jurors; the per-juror configurations are recorded in the replication archive export",
            compiledEv ?? createdEv,
          ),
        );
      } else {
        ...
        const decode = [
          typeof params.temperature === "number"
            ...
            : "a temperature that was not recorded",
          typeof params.maxTokens === "number"
            ? `a maximum of ${params.maxTokens} output tokens`
            : "an output-token maximum that was not recorded",
        ];
        ...
        const seedNonTransmitting = seedProviders.filter(
          (pr) => !SEED_TRANSMITTING_PROVIDERS.has(pr),
        );
        if (
          params.seed !== undefined &&
          seedProviders.length > 0 &&
          seedNonTransmitting.length === 0
        ) {
        ...
        if (
          params.seed !== undefined &&
          (seedProviders.length === 0 || seedNonTransmitting.length > 0)
        ) {
      ...
      const templates = jurors
        .map((j) => j?.promptTemplate ?? p.promptTemplate)
        .filter((t) => typeof t === "string");
      const allSlots =
        templates.length > 0 &&
        templates.every((t) =>
          ["{{definition}}", "{{criteria}}", "{{examples}}"].every((slot) => t.includes(slot)),
        );
      if (allSlots) {
        sentences.push(
          s(
            "The compiled codebook (definition, criteria and worked examples) was injected into the prompt verbatim",
            compiledEv ?? createdEv,
          ),
        );
      }
      if (jurors[0]?.rationaleFirst) {
        sentences.push(
          s(
            "Each judgment required a written rationale before the verdict and was returned as schema-constrained structured output",
            compiledEv ?? createdEv,
          ),
        );
      }
      ...
      sentences.push(
        s(
          `Measurement used a closed-vocabulary dictionary instrument with ${cats.length} ${cats.length === 1 ? "category" : "categories"} and ${terms} terms, scored as ${p.scoring ?? "percentOfWords"}${p.negation?.enabled ? ` with a ${p.negation.window}-token negation window` : ""}, executed locally without any model call`,
          createdEv,
        ),
      );
    } else {
    ...
    sentences.push(
      s(
        "The complete instrument specification, including the full prompt text where applicable, is available in the replication archive export accompanying this report",
        anchorEvent,
      ),
    );
      ...
      sentences.push(
        s(
          tunedFirst
            ...
            : `${tuned.charAt(0).toUpperCase()}${tuned.slice(1)}; the ledger does not establish whether tuning preceded human validation`,
          silverEv ?? createdEv,
        ),
      );
    }
    if (instrument.stability) {
      sentences.push(
        s(
          `A ${instrument.stability.k}-run test-retest stability check on ${instrument.stability.n} units yielded alpha = ${fmt(instrument.stability.alpha)}`,
          stabilityEv ?? anchorEvent,
        ),
      );
    }
    if (instrument.frozen) {
      sentences.push(
        s(
          `The instrument was frozen as version ${instrument.version} with content hash ${instrument.versionHash.slice(0, 12)}…, so any subsequent edit forks a new version rather than altering the version reported here`,
          frozenEv ?? anchorEvent,
        ),
      );
    }
    ...
    if (run) {
      ...
      if (run.pinned && instrument.kind === "dictionary") {
        sentences.push(
          s(
            "The dictionary instrument executed locally with no model call; the run records the instrument version hash as its snapshot, pinned for every call in this run",
            runStartedEv ?? anchorEvent,
          ),
        );
      } else if (run.pinned) {
        ...
        const snaps = [
          ...new Set(
            jurorList.map((j) => j?.snapshot).filter((x) => typeof x === "string" && x.length > 0),
          ),
        ];
        const everyRecorded =
          jurorList.length > 0 &&
          jurorList.every((j) => typeof j?.snapshot === "string" && j.snapshot.length > 0);
        if (everyRecorded) {
          sentences.push(
            s(
              `A model snapshot identifier was recorded for every juror (${snaps.map(inline).join(", ")}) and is reported as pinned for every call in this run; Concord records but does not verify that the provider served ${snaps.length === 1 ? "that snapshot" : "those snapshots"}`,
              runStartedEv ?? anchorEvent,
            ),
          );
          ...
          sentences.push(
            s(
              `A model snapshot identifier was recorded (${inline(run.snapshot)}) and is reported as pinned for every call in this run; Concord records but does not verify that the provider served that snapshot`,
              runStartedEv ?? anchorEvent,
            ),
          );
        } else {
          sentences.push(
            s(
              "The run reports its model snapshot as pinned for every call in this run, but no snapshot identifier was recorded; this gap should be weighed when interpreting reproducibility",
              runStartedEv ?? anchorEvent,
            ),
          );
        }
        ...
        sentences.push(
          s(
            "The run did not pin a model snapshot; the recorded snapshot string identifies the model as served at run time, and this limitation should be weighed when interpreting reproducibility",
            runStartedEv ?? anchorEvent,
          ),
        );
      }
  ...
  if (cert?.agreement) {
    ...
    if (typeof a.alpha === "number")
      bits.push(`alpha = ${fmt(a.alpha)}${fmtCI(a.ci) ? `, 95% CI ${fmtCI(a.ci)}` : ""}`);
    para(
      s(
        `Against the adjudicated gold standard (n = ${a.n}), the frozen instrument achieved ${bits.join(", ")}`,
        frozenEv ?? anchorEvent,
      ),
    );
    if (Array.isArray(a.perClass) && a.perClass.length > 0) {
      para(
        s(
          "Per-class operating characteristics of the frozen instrument were as follows",
          frozenEv ?? anchorEvent,
        ),
      );
      ...
      for (const r of a.perClass) {
        out.push(
          `| ${inline(String(r.label))} | ${fmt(r.precision)} | ${fmt(r.recall)} | ${fmt(r.f1)} | ${r.support} |`,
        );
      }
      out.push("");
    }
  ...
  if (instrument?.kind === "panel" || run) {
      ...
      sentences.push(
        s(
          agg
            ...
            : "Juror outputs were aggregated per unit; the aggregation rule was not recorded",
          find("instrument.compiled", instrument.id) ?? anchorEvent,
        ),
      );
    } else if (instrument) {
      ...
      sentences.push(
        s(
          run?.escalation?.count === 0
            ...
            : "Machine labels came from the instrument with no panel aggregation",
          anchorEvent,
        ),
      );
    }
    if (run) {
      ...
      const cost =
        typeof run.cost?.actualUSD === "number"
      ...
      sentences.push(
        s(
          `The instrument was applied to the corpus in run ${run.id}, ${progress}, ${cost}`,
          completedEv ?? startedEv ?? anchorEvent,
        ),
      );
        ...
        sentences.push(
          s(
            dm
              ...
              : `${count(escN, "unit")} ${escN === 1 ? "was" : "were"} escalated: the primary judgment was flagged by the escalation predicate (low confidence, high entropy, schema repair, or unusually long text); the run names no Director model (not recorded), so flags carry no second opinion and escalated units keep the primary judgment`,
            completedEv ?? anchorEvent,
          ),
        );
      }
      if (run.quarantine?.length > 0) {
        sentences.push(
          s(
            `${count(run.quarantine.length, "unit")} failed structured-output enforcement (schema validation after constrained repair, provider refusal, or truncation) and ${run.quarantine.length === 1 ? "was" : "were"} quarantined rather than silently dropped`,
            completedEv ?? anchorEvent,
          ),
        );
      }
  ...
  if (isDsl) {
    ...
    const estimatorName = /^ppi/.test(estimator)
    ...
    sentences.push(
      s(
        `Population estimates were corrected for machine misclassification with ${estimatorName}, combining the machine labels on every unit with the adjudicated gold labels on ${design} whose inclusion probabilities were recorded at sampling time (Egami, Hinck, Stewart, and Wei 2023; Angelopoulos, Bates, Fannjiang, Jordan and Zrnic 2023)`,
        analysisEv,
      ),
    );
    if (goldset?.design === "uncertainty") {
      sentences.push(
        s(
          "The gold sample was uncertainty-targeted: its recorded inclusion probabilities are nominal n/N over a deterministic uncertainty ranking, so the design-based unbiasedness guarantee of this estimator does not apply",
          analysisEv,
        ),
      );
    }
    sentences.push(
      s(
        "For each estimand the gold-corrected pseudo-outcome replaces the raw machine label in the usual moment condition, and sandwich standard errors with normal-quantile 95% confidence intervals are reported",
        analysisEv,
      ),
    );
    sentences.push(
      s(
        "The uncorrected naive estimate is reported beside every corrected value, never instead of it",
        analysisEv,
      ),
    );
    if (results.diff && typeof results.diff.est === "number") {
      ...
      sentences.push(
        s(
          `The corrected ${inline(results.outcome ?? "outcome")} difference between ${inline(d.a)} and ${inline(d.b)} was ${fmt(d.est, 3)} (SE ${fmt(d.se, 3)}), 95% CI [${fmt(d.ciLo, 3)}, ${fmt(d.ciHi, 3)}], against a naive difference of ${fmt(d.naive?.est, 3)}`,
          analysisEv,
        ),
      );
    } else if (
      ...
      results.cells[0] &&
      typeof results.cells[0].est === "number"
    ) {
      ...
      const naiveBit =
        c.naive && typeof c.naive.est === "number"
          ? `, against a naive estimate of ${fmt(c.naive.est, 3)}`
          : ", with no naive companion recorded";
      sentences.push(
        s(
          `The corrected estimate for ${inline(String(c.group))} was ${fmt(c.est, 3)} (SE ${fmt(c.se, 3)}), 95% CI [${fmt(c.ciLo, 3)}, ${fmt(c.ciHi, 3)}]${naiveBit}`,
          analysisEv,
        ),
      );
    }
    sentences.push(
      s(
        `This analysis carries the ${levelName} (${mark}) evidence mark, the highest level on Concord's evidence ladder`,
        analysisEv,
      ),
    );
    para(...sentences);
