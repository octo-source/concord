...
export function render(mount, params) {
  asyncMount(
    ...
    async () => {
      ...
      const analysis = await api.analyses.create(params.slug, {
        kind: "descriptive",
        spec: { runId: params.runId },
      });
    ...
    ({ project, analysis }) => {
        ...
        mount.append(
          emptyState({
            ...
            actions: [
              el(
                "a",
                { class: "btn", href: `#/p/${params.slug}/runs/${params.runId}` },
      ...
      const corpusEntry =
        (project?.corpora ?? []).find((c) => c.id === scopeCorpusId) ??
        project?.corpora?.[0] ??
        null;
      mount.append(
        contextLine([
      ...
      if (co?.labels?.length && Array.isArray(co.matrix)) {
        ...
        heat.render(
          ...
          {
            caption:
              "Theme co-occurrence — units carrying both labels. Cells with units are doors.",
            format: (v) => String(v),
          },
        );
      ...
      if (nudge) {
        ...
        mount.append(
          levelUpNudge({
            ...
            price:
              nudge.estUnits !== undefined
                ? `~${fmtCount(nudge.estUnits)} units, ~${fmtCount(nudge.estMinutes)} min`
                : nudge.price,
            onGo: () =>
              router.navigate(
                goldsetId ? `p/${params.slug}/goldsets/${goldsetId}` : `p/${params.slug}`,
              ),
          }),
        );
      }
      ...
      mount.append(
        el(
          ...
          el(
            "a",
            { href: `#/p/${params.slug}/analyses?runId=${encodeURIComponent(params.runId)}` },
            "Analyze in the Workbench →",
          ),
          el(
            "span",
            { class: "faint" },
            " crosstabs, models, triangulation over this run — corrected where gold exists.",
          ),
        ),
      );
    },
