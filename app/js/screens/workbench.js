...
const KINDS = [
  { value: "descriptive", label: "Descriptive", hint: "prevalence, distributions" },
  {
    value: "crosstab",
    label: "Crosstab",
    hint: "construct × metadata, χ² honesty, DSL when gold exists",
  },
  { value: "model", label: "Model", hint: "OLS / logistic with sandwich SEs" },
  {
    value: "triangulation",
    label: "Triangulation",
    hint: "agreement between two instruments, with the units they label differently",
  },
  {
    value: "subgroup",
    label: "Subgroup audit",
    hint: "machine-vs-gold agreement and error rates by group — needs a complete gold set",
  },
];
  ...
  asyncMount(
    ...
    async () => {
      ...
      const columns = columnsCorpusId
        ? await api.corpora
            .columns(params.slug, columnsCorpusId)
    ...
    ({ project, analysis, columns, columnsCorpusId }) => {
      ...
      const presetRunId =
        query?.runId && (project.runs ?? []).some((r) => r.id === query.runId) ? query.runId : null;
      ...
      if (project.analyses?.length) {
        rail.append(
          el("h3", { class: "overline split__group" }, "On the bench"),
      ...
      if (analysis?.results) {
        ...
        canvas.append(
          emptyState({ title: "Analysis not found.", body: "It may not have been computed yet." }),
        );
      }
    },
...
function builderRail(
  ...
  const runOptionLabel = (r) => {
    ...
    if (!base) {
      const instName =
        instruments.find((i) => i.id === r.instrumentId)?.name ?? r.instrumentId ?? r.id;
      const corpus = (project.corpora ?? []).find((c) => c.id === r.corpusId) ?? null;
      const corpusName = corpus
        ? scopechip.displayName(corpus)
        : (r.corpusId ?? "corpus not recorded");
  ...
  const runSel = el(
    "select",
    { class: "input", "aria-label": "Run whose outputs new analyses read" },
    ...completeRuns.map((r) =>
      el("option", { value: r.id, selected: r.id === selectedRunId }, runOptionLabel(r)),
    ),
  );
  ...
  const columnSelect = (
    ariaLabel,
    cols,
    { lead = null, why = "no metadata columns detected" } = {},
  ) => {
    const options = [];
    ...
    if (cols.length)
      options.push(
        ...cols.map((c, i) =>
          el("option", { value: c.name, selected: !lead && i === 0 }, columnOptionLabel(c)),
        ),
      );
  ...
  const constructSel = el(
    "select",
    { class: "input", "aria-label": "Construct" },
  ...
  const instASel = el(
    "select",
    { class: "input", "aria-label": "Instrument A" },
    ...instruments.map((i) => el("option", { value: i.id }, i.name)),
  );
  const instBSel = el(
    "select",
    { class: "input", "aria-label": "Instrument B" },
  ...
  if (!hasRuns) {
    for (const sel of [constructSel, rowSel, colSel, bySel, xSel, instASel, instBSel])
      sel.disabled = true;
  }
  ...
  const columnsLine =
    columnsCorpus && columns.length
      ? el(
          "p",
          { class: "screen__hint faint" },
          "Variables are ",
          el("span", { class: "data" }, scopechip.displayName(columnsCorpus)),
  ...
  const missingVariable = () => {
    ...
    if (kind === "triangulation" && instruments.length < 2)
      return "triangulation needs two instruments";
    if (kind !== "crosstab" && kind !== "triangulation" && constructs.length === 0)
      return "write a construct first";
    return null;
  };
  const paintRunState = () => {
  ...
  const kindList = el(
    "div",
    { class: "choicelist", role: "radiogroup", aria: { label: "Analysis kind" } },
    ...KINDS.map((k) =>
      el(
        "label",
        { class: "choice" },
      ...
      onclick: async () => {
        ...
        clear(canvas).append(
          el(
            ...
            "computing — DSL applies automatically where gold with π exists…",
          ),
        );
        try {
          ...
          clear(canvas);
          renderResult(canvas, params, analysis, project);
          toast.success("Analysis computed.", {
            detail: `${kind} · ${analysis.level}`,
            data: true,
          });
        } catch (err) {
          clear(canvas).append(
            emptyState({ title: "The analysis failed.", body: String(err.message ?? err) }),
          );
        }
  ...
  if (typeof analysis.results?.note === "string" && analysis.results.note) {
    canvas.append(
      el(
        ...
        el("span", { class: "chip chip--signal" }, "note"),
        " ",
...
function descriptiveResult(canvas, analysis, project = null) {
  ...
  const instrument =
    (project?.instruments ?? []).find((i) => i.id === analysis.spec?.instrumentId) ?? null;
  ...
  bar.render(
    ...
    {
      caption:
        `Raw machine-label shares over ${fmtCount(r.n)} units — every bar opens its units` +
      ...
      section(
        ...
        el(
          ...
          el(
            ...
            el(
              "caption",
              { class: "sr-only" },
              `${rowKey ?? "row"} × ${colKey ?? "col"} contingency table`,
            ),
            el(
              "thead",
              {},
              el(
                ...
                ...t.cols.map((c) =>
                  el("th", { scope: "col", class: "table__num data" }, String(c)),
                ),
            ...
            el(
                      ...
                      ids
                        ? // evidenceTotal: the TRUE cell count — evidence ids cap at
                          ...
                          el(
                            "button",
                            {
                              class: "evidence-door table__doorbtn",
                              type: "button",
                              dataset: {
                                evidence: ids.join(","),
                                evidenceTotal: String(t.matrix[i][j]),
                              },
                            },
                            text,
                          )
              ...
              el(
                ...
                el("th", { scope: "row" }, "Σ"),
                ...t.cols.map((_, j) =>
                  el("td", { class: "table__num data" }, fmtCount(t.colTotals?.[j])),
                ),
          ...
          (t.minExpected ?? 99) < 5
            ? el(
                "span",
                {
                  ...
                  title:
                    "Expected cell counts under 5 degrade the χ² approximation — a standard rule of thumb (Cochran's), not a hard gate",
                },
                "small-n warning",
              )
            : null,
        ...
        ...(r.warnings ?? []).map((w) =>
          el(
            "p",
            { class: "annotation annotation--still" },
            el("span", { class: "chip chip--signal" }, w.kind ?? "note"),
            " ",
            w.message ?? String(w),
          ),
        ),
  ...
  if (level === "corrected" && r.cells?.length) {
    canvas.append(
      el(
        "p",
        { class: "annotation annotation--still" },
        "The table and χ² above are computed on ",
        el("strong", {}, "raw machine labels"),
  ...
  if (!(level === "corrected" && r.cells?.length)) {
    canvas.append(
      el(
        ...
        { class: "annotation annotation--still" },
        "These cells are ",
        el("strong", {}, "uncorrected"),
        " (",
        ladderC.mark(level),
        " ",
        level,
        "). A gold sample with stored π would let design-based supervised learning (DSL",
        cite("egami2023"),
        ") remove machine-error bias — the uncorrected level mark stays on these numbers in every export until a gold sample with π is provided.",
      ),
    );
  }
}
...
function correctedCellsBlock(canvas, analysis) {
  ...
  bar.render(
    ...
    r.cells.map((c) => ({
      ...
      evidence: positive
        ? (doorIds(analysis, `${positive}|${c.group}`) ?? undefined)
        : (doorIds(analysis, String(c.group)) ?? undefined),
    })),
    {
      paired: true,
  ...
  canvas.append(
    section(
      ...
      table.render({
        ...
        columns: [
          { key: "group", label: r.groupBy ?? "group" },
          {
            ...
            evidence: (row) =>
              positive
                ? doorIds(analysis, `${positive}|${row.group}`)
                : doorIds(analysis, String(row.group)),
          },
          {
            key: "ciLo",
            label: "95% CI",
            numeric: true,
            sortable: false,
            format: (v, row) => `[${fmtStat(row.ciLo)}, ${fmtStat(row.ciHi)}]`,
          },
      ...
      el(
        ...
        " Corrected for machine-labeling error using the gold sample — design-based supervised learning (DSL",
        cite("egami2023"),
        "), of the prediction-powered-inference family",
      ...
      ...(r.skippedGroups ?? []).map((s) =>
        el(
          "p",
          { class: "annotation annotation--still faint" },
          el("span", { class: "chip chip--ghost" }, s.group),
          " ",
          s.reason,
        ),
      ),
...
function coefficientForest(analysis) {
  ...
  const plotted = hasSlopes
    ? coef.filter((c) => !INTERCEPT_NAMES.has(String(c.name).toLowerCase()))
    : coef;
  ...
  const rows = plotted.map((c) => {
    ...
    return {
      ...
      corrected: {
        value: c.est,
        ...(se !== null && se > 0 ? { ci: [c.est - 1.96 * se, c.est + 1.96 * se] } : {}),
      },
      ...
      el(
        ...
        r.n
          ? el(
              "span",
              { class: "data faint" },
              ` · n = ${fmtCount(r.n)}${r.nGold !== undefined ? `, gold = ${fmtCount(r.nGold)}` : ""}`,
            )
          : null,
        r.converged === false
          ? el("span", { class: "chip chip--signal" }, "did not converge")
          : null,
      ),
      ...
      table.render({
        ...
        columns: [
          ...
          { key: "z", label: "z", numeric: true, format: zp },
          {
            ...
            label: "p",
            numeric: true,
            format: (v) => (v === null || v === undefined ? "—" : fmtP(v)),
          },
        ],
        rows: coef,
      ...
      ...coef
        ...
        .map((c) =>
          el(
            "p",
            { class: "annotation annotation--still" },
            el("span", { class: "chip chip--ghost" }, c.name),
            " ",
            c.note,
          ),
        ),
...
function triangulationResult(canvas, params, analysis) {
  ...
  canvas.append(
    section(
      ...
      el(
        "p",
        { class: "screen__hint" },
        el("span", { class: "data" }, a?.name ?? "A"),
        " vs ",
        el("span", { class: "data" }, b?.name ?? "B"),
        el(
          "span",
          { class: "data faint" },
  ...
  const numericPairs = (r.pairs ?? []).filter(
    (p) => typeof p.a === "number" && typeof p.b === "number",
  );
  if (numericPairs.length >= 3) {
    const cell = el("div", {});
    scatter.render(
      cell,
      numericPairs.map((p) => ({ x: p.a, y: p.b, label: p.unitId, id: p.unitId })),
      {
  ...
  if (!divergent.length) {
    browser.append(
      el(
        "p",
        { class: "faint" },
        "No divergent units. The two instruments assigned the same labels for every jointly labeled unit.",
      ),
    );
  ...
  for (const d of divergent.slice(0, 25)) {
    const row = el(
      ...
      el(
        ...
        el(
          "button",
          { class: "refchip data evidence-door", type: "button", dataset: { evidence: d.unitId } },
          String(d.unitId).slice(0, 10) + "…",
        ),
  ...
  canvas.append(
    section(
      `Divergence browser — ${fmtCount(divergentN)} units the instruments label differently`,
      browser,
    ),
  );
}
...
function subgroupResult(canvas, analysis) {
  ...
  const agreementCell = (v, row) => {
    if (v === null || v === undefined)
      return el("span", { class: "faint", title: row.note ?? "no gold in this group" }, "—");
    return el(
      "span",
      { class: "subgroupflag" },
      ...
      row.flagged
        ? el(
            "span",
            {
              class: "chip chip--signal",
              title:
                "agreement sits >0.1 below the overall — read this group before trusting its numbers",
            },
            "flagged",
          )
        : null,
  ...
  const audited =
    groups.some((g) => g.goldN !== undefined) || (r.overall && typeof r.overall === "object");
  const noted = groups.filter((g) => g.note);
  const flaggedN = groups.filter((g) => g.flagged).length;

  canvas.append(
    section(
      "Subgroup reliability audit",
      el(
        "p",
        { class: "screen__hint" },
        ...
        r.overall
          ? el(
              ...
              r.overall.kappa !== null && r.overall.kappa !== undefined
                ? ` · κ ${fmtStat(r.overall.kappa)}`
                : "",
      ...
      table.render({
        ...
        columns: [
          { key: "group", label: r.by ?? "group" },
          {
            ...
            numeric: true,
            format: (v) => fmtCount(v),
            evidence: (row) => doorIds(analysis, String(row.group)),
          },
          {
            key: "goldN",
            label: "gold n",
            numeric: true,
            format: (v) => (v === undefined ? "—" : fmtCount(v)),
          },
          ...
          { key: "dist", label: "top labels", sortable: false, format: (v) => distLine(v) },
          {
            ...
            label: "corrected ◉",
            numeric: true,
            format: (v) => (v ? fmtStat(v.est) : "—"),
            level: (row) => (row.corrected ? "corrected" : null),
          },
          {
            key: "corrected",
            label: "95% CI",
            numeric: true,
            sortable: false,
            format: (v) => (v ? `[${fmtStat(v.ciLo)}, ${fmtStat(v.ciHi)}]` : "—"),
          },
        ],
        rows: groups,
        empty: {
      ...
      ...noted.map((g) =>
        el(
          "p",
          { class: "annotation annotation--still faint" },
          el("span", { class: "chip chip--ghost" }, g.group),
          " ",
          g.note,
        ),
      ),
...
async function addToReport(params, analysis) {
  ...
  try {
    ...
    if (cached?.slug === params.slug) {
      ...
      cached.report.blocks = [
        ...(cached.report.blocks ?? []),
        { ...block, addedAt: new Date().toISOString() },
      ];
