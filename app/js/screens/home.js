...
const PRIVACY = [
  { value: "open", label: "Open", hint: "Any configured model backend may read this corpus." },
  {
    value: "no-training",
    label: "No-training",
    hint: "Only backends with contractual no-training terms (plus local models). Overrides are logged.",
  },
  {
    value: "strict",
    label: "Strict · local only",
    hint: "Network adapters are disabled app-wide. The Director must be a local model.",
  },
];
      ...
      mount.append(
        screenHead({
          ...
          actions: [
            el(
              "button",
              { class: "btn btn--primary", type: "button", onclick: () => newProjectSheet() },
              "New project",
            ),
          ],
        }),
...
function projectCard(p) {
  if (p.corrupt) {
    return el(
      "div",
      { class: "projcard projcard--corrupt" },
  ...
  const workRow = el(
    "p",
    { class: "projcard__ladder data faint" },
  ...
  return el(
    "a",
    { class: "projcard", href: `#/p/${encodeURIComponent(p.slug)}` },
    el("h3", { class: "projcard__name" }, p.name),
    el(
      "p",
      { class: "projcard__meta data" },
...
function newProjectSheet() {
  const name = el("input", {
    class: "input",
    type: "text",
    placeholder: "e.g. TechCorp Exit Survey",
  ...
  s.body.append(
    el("label", { class: "field" }, el("span", { class: "field__label overline" }, "Name"), name),
    el(
      "div",
      { class: "field" },
      ...
      el(
        "p",
        { class: "field__hint" },
        "Privacy is enforced at the provider layer, not by promise. You can tighten it later; loosening asks for explicit confirmation and is ledgered.",
      ),
      radios,
    ),
  );
  ...
  ensureProject(params.slug)
    .then((project) => {
      if (!project.corpora?.length) router.navigate(`p/${params.slug}/import`, { replace: true });
        ...
        router.navigate(`p/${params.slug}/corpus/${project.corpora[0].id}/instant`, {
          replace: true,
        });
    })
    .catch(() => {
