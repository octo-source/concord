...
function makePreviewScope(project) {
  ...
  return {
    ...
    corpus() {
      return corpora.find((c) => c.id === this.corpusId) ?? null;
    },
    set(id) {
      this.corpusId = id;
...
function editorContextLine(params, inst, construct, previewScope) {
  ...
  if (corpora.length > 1) {
    readsPart = {
      ...
      node: el(
        "select",
        {
          ...
          "aria-label":
            "Corpus the previews and checks read — picking a corpus picks the text column",
          onchange: (e) => previewScope.set(e.target.value),
        },
        ...corpora.map((c) =>
          el(
            "option",
            { value: c.id, selected: c.id === previewScope.corpusId },
            scopechip.optionLabel(c),
          ),
        ),
      ),
    };
    ...
    readsPart = {
      label: "reads",
      text: "no corpus yet — import one to preview",
  ...
  return el(
    ...
    contextLine([
      construct
        ? {
            label: "measures",
            text: construct.name,
            href: `#/p/${params.slug}/constructs/${construct.id}`,
          }
    ...
    el(
      "p",
      { class: "screen__hint faint readscope__hint" },
...
function readLine(corpus, n) {
  return el(
    ...
    "Read ",
    el("span", { class: "data" }, fmtCount(n)),
    " units from ",
    el("span", { class: "data" }, corpus ? scopechip.displayName(corpus) : "—"),
    " (text: ",
    el("span", { class: "data" }, textColumnOf(corpus) ?? "not recorded"),
    ").",
  );
}
...
export function failurePanel(quarantine, total) {
  ...
  if (reasons.length === 1 && reasonText(reasons[0])) {
    ...
    head.append(
      " — ",
      el("span", { class: "faint" }, "no failure reasons recorded (older server)"),
    );
  }

  return el(
    "div",
    { class: "failpanel", role: "alert" },
    head,
    reasons.length > 1
      ? el(
          "ul",
          { class: "failpanel__lines", role: "list" },
          ...reasons.map((g) =>
            el(
              "li",
              { class: "failpanel__line" },
              el("span", { class: "data failpanel__count" }, `×${fmtCount(g.count)}`),
              " ",
              reasonText(g) ||
                el("span", { class: "faint" }, "no failure reason recorded (older server)"),
            ),
          ),
        )
      : null,
    ...
    ({ project, instruments, constructs, catalog }) => {
          ...
          actions: [
            el(
              "button",
              {
                ...
                type: "button",
                onclick: () => openCompile(query?.construct ?? null),
              },
              "New instrument…",
            ),
          ],
      ...
      mount.append(
        el(
          ...
          "A ",
          el("strong", {}, "judge"),
          " is a model given your codebook as instructions. A ",
          el("strong", {}, "dictionary"),
          " is a transparent term list — instant and free. A ",
          el("strong", {}, "panel"),
          " is several judges that vote. Most studies start by compiling a judge from a construct.",
        ),
      );
          ...
          emptyState({
            ...
            actions: [
              constructs.length
                ...
                : el(
                    "a",
                    { class: "btn btn--primary", href: `#/p/${params.slug}/constructs` },
                    "Write a construct first",
                  ),
            ],
          }),
        );
              ...
              el(
                ...
                el(
                  ...
                  glyph.render({
                    ...
                    humanTouched: inst.humanTouched ?? true,
                  }),
                ),
                el(
                  "span",
                  ...
                  actions: [
                    el(
                      "button",
                      {
                        class: "btn btn--primary",
                        type: "button",
                        onclick: () => openCompile(wanted.id),
                      },
                      `Compile a judge for “${wanted.name}”`,
                    ),
                  ],
                }
              : {
    ...
    destroy() {
      while (disposers.length) {
        ...
        try {
          ...
          console.error("instrument editor disposer threw", err);
        }
      }
    },
  };
...
function instrumentEditor(
  ...
  const paintStrip = () => {
    ...
    const hasCompleteRun = (project?.runs ?? []).some(
      (r) => r.instrumentId === inst.id && r.status === "complete",
    );
    ...
    const companion = {
      ...
      title:
        "Every reading of this construct and how much the readers agree — κ, α, and the gold anchor live here",
    };
    let action;
    let secondary = null;
    ...
    clear(stripHost).append(
      pipeline.render({ current: "instrument", states, action, secondary, companion }),
    );
  };
      ...
      onclick: async () => {
        ...
        try {
          ...
          toast.success("Instrument saved.", {
            detail:
              "edits reset its evidence level to ◌ — run Silver-tune (which reruns the stability check) or calibrate against gold to restore it",
          });
          dirty = false;
        } catch (err) {
    ...
    el(
      ...
      el(
        ...
        el(
          ...
          el(
            "span",
            { class: "chip chip--ghost data", title: "Content-addressed version hash" },
            String(inst.versionHash ?? "").slice(0, 10),
          ),
          inst.stability
            ? el(
                "a",
                {
                  ...
                  href: inst.stability.corpusId
                    ? `${reliabilityHref}?corpusId=${encodeURIComponent(inst.stability.corpusId)}`
                    : reliabilityHref,
        ...
        inst.kind === "judge"
          ? el(
              "button",
              {
                ...
                type: "button",
                title:
                  "Create a separate instrument with this compiled prompt and another model — calibration does not transfer between models",
  ...
  if (inst.frozen) {
    main.append(
      el(
        "p",
        { class: "screen__hint annotation annotation--still" },
  ...
  if (inst.kind === "dictionary")
    dictionaryEditor(main, params, inst, touch, () => dirty, previewScope, disposers);
  ...
  if (inst.certificate) {
    main.append(
      section("Calibration certificate", certificateCard(inst.certificate, { reliabilityHref })),
    );
  }
...
function duplicateSheet(params, inst, catalog) {
  const s = openSheet({
    title: "Duplicate with another model",
    overline: "Same prompt, different judge",
  });
  let choice = null;
  ...
  s.body.append(
    ...
    el(
      "div",
      { class: "field" },
      el("span", { class: "field__label overline" }, "Judge with"),
      picker.el,
    ),
  );
  const createBtn = el(
    "button",
    {
      ...
      onclick: async () => {
        ...
        try {
          ...
          s.close();
          toast.success(
            "Created as a separate instrument — calibration does not transfer between models. Gold-test it before trusting it.",
          );
          await refreshProject(params.slug).catch(() => {});
          router.navigate(`p/${params.slug}/instruments/${created.id}`);
        } catch (err) {
          createBtn.disabled = false;
          toast.error("Could not duplicate the instrument.", {
...
function dictionaryEditor(
    ...
    payload.categories.forEach((cat, ci) => {
      ...
      const chips = el(
        ...
        ...cat.terms.map((t, ti) =>
          el(
            "span",
            { class: "termchip chip chip--cat", dataset: { slot } },
            el("span", { class: "termchip__term" }, t.term),
            t.weight !== undefined && t.weight !== 1
              ? el("span", { class: "termchip__weight data" }, `×${t.weight}`)
              : null,
            !ro
              ? el(
                  "button",
        ...
        !ro
          ? el("input", {
              class: "input termchips__add",
              placeholder: '+ term, underpa*, "exact phrase"',
            ...
            !ro
              ? el(
                  "button",
                  {
                    ...
                    onclick: () => {
                      payload.categories.splice(ci, 1);
                      touch();
                      redraw();
                      schedulePreview();
                    },
        ...
        el(
          "button",
          {
            ...
            onclick: () => {
              ...
              if (name) {
                payload.categories.push({ name, terms: [] });
                touch();
                redraw();
              }
            },
  ...
  const negWindow = el("input", {
    ...
    min: 1,
    max: 8,
    value: payload.negation?.window ?? 3,
    disabled: ro || !payload.negation?.enabled,
    "aria-label": "Negation window in tokens",
    onchange: (e) => {
      payload.negation.window = Number(e.target.value);
      touch();
      schedulePreview();
    },
  });
    ...
    section(
      ...
      el(
        ...
        el(
          ...
          el("input", {
            type: "checkbox",
            checked: Boolean(payload.negation?.enabled),
            disabled: ro,
            onchange: (e) => {
        ...
        el(
          ...
          el(
            "select",
            {
              ...
              onchange: (e) => {
                payload.scoring = e.target.value;
                touch();
                schedulePreview();
              },
            },
  ...
  async function runPreview() {
    ...
    clear(previewWrap).append(
      el("p", { class: "faint" }, draft ? "scoring the draft locally…" : "previewing…"),
    );
    try {
      const corpus = previewScope?.corpus() ?? null;
      ...
      const quarantined = new Set(
        draft ? [] : normalizeQuarantine(res?.quarantine).map((q) => q.unitId),
      );
      if (draft) {
        previewWrap.append(
          el(
            "p",
            { class: "dictpreview__draftnote faint" },
      ...
      for (const unit of units) {
        ...
        const out = draft
          ? null
          : outputs.find((o) => o.unitId === unit.id && o.label !== undefined);
        const hits = draft ? localHits(unit.text, inst.payload) : (out?.hits ?? []);
        previewWrap.append(
          el(
            "div",
            { class: "dictpreview__row" },
            quotecard.render({ unit, highlights: hits, compact: true, evidence: true }),
            out?.scores
              ? el(
                  "p",
                  { class: "dictpreview__scores data" },
                  Object.entries(out.scores)
                    .filter(([, v]) => v > 0)
                    .map(([k, v]) => `${k} ${fmt(v, 1)}%`)
                    .join(" · ") || "no hits",
                )
      ...
      clear(previewWrap).append(
        el("p", { class: "faint" }, "Preview unavailable: ", String(err.message ?? err)),
      );
    }
  }
  runPreview();
...
function judgeEditor(main, params, inst, catalog, construct, touch) {
  ...
  const promptView = el("pre", {
    class: "promptview",
    tabindex: "0",
    aria: { label: "Compiled prompt template" },
  });
  const paintPrompt = () => {
    clear(promptView);
  ...
  const rawArea = el(
    "textarea",
    {
      class: "input textarea promptraw",
      rows: 12,
      disabled: ro,
      "aria-label": "Raw prompt template",
      oninput: (e) => {
        payload.promptTemplate = e.target.value;
        touch();
        paintPrompt();
      },
    },
  ...
  const rawReveal = el(
    ...
    el(
      "p",
      { class: "screen__hint faint" },
      "The compiled view is what runs; edit the raw template here only if you need to. Slots ",
      el("code", {}, "{{definition}} {{criteria}} {{examples}}"),
      " fill from the construct, ",
      el("code", {}, "{{unit}}"),
      " from each unit, at call time.",
    ),
    rawArea,
  );
  ...
  main.append(
    section(
      ...
      el(
        ...
        { class: "screen__hint faint" },
        "Compiled from ",
        el("strong", {}, construct?.name ?? inst.constructId),
        ...
        el(
          ...
          paramControl(
            ...
            el("input", {
              ...
              max: 2,
              disabled: ro,
              value: payload.params?.temperature ?? 0,
              "aria-label": "Temperature",
              onchange: (e) => {
                payload.params = payload.params ?? {};
                payload.params.temperature = Number(e.target.value);
                touch();
              },
            }),
          ...
          paramControl(
            ...
            el("input", {
              ...
              min: 50,
              disabled: ro,
              value: payload.params?.maxTokens ?? 400,
              "aria-label": "Max tokens",
              onchange: (e) => {
                payload.params = payload.params ?? {};
                payload.params.maxTokens = Number(e.target.value);
                touch();
              },
            }),
        ...
        el(
          ...
          el(
            "select",
            {
              class: "input input--inline",
              disabled: ro,
              "aria-label": "Worker class",
              onchange: (e) => {
                payload.workerClass = e.target.value;
                touch();
              },
            },
          ...
          el(
            "span",
            { class: "faint" },
            "smaller classes get more rubric anchoring when the Director compiles",
          ),
        ),
        el(
          "div",
...
function paramControl(labelText, input, supported) {
  ...
  return el(
    "div",
    { class: `paramctl${ignored ? " paramctl--ignored" : ""}` },
    el("label", { class: "controlrow__item" }, el("span", { class: "overline" }, labelText), input),
    ignored
      ? el("p", { class: "paramctl__note" }, "ignored by this model (reasoning-class)")
      : null,
  );
}
    ...
    payload.jurors.forEach((j, i) => {
      ...
      jurorWrap.append(
        el(
          ...
          el(
            "p",
            { class: "juror__meta faint data" },
            `temperature ${j.params?.temperature ?? 0} · max ${j.params?.maxTokens ?? 400} tokens`,
          ),
          noTemp
            ? el(
                "p",
                { class: "paramctl__note" },
                "temperature ignored by this model (reasoning-class)",
              )
            : null,
          !ro
            ? el(
                "button",
                {
                  class: "btn btn--quiet juror__remove",
                  type: "button",
                  aria: { label: `Remove juror ${j.model}` },
                  onclick: () => {
                    payload.jurors.splice(i, 1);
                    touch();
                    redraw();
                  },
                },
  ...
  const paintWarnAndCost = () => {
    ...
    if (dupes.length) {
      warnBox.append(
        el(
          "p",
          { class: "annotation annotation--signal" },
    ...
    if (payload.jurors.length > 0 && payload.jurors.length < 3) {
      warnBox.append(
        el(
          ...
          "Panels run best at 3–5 jurors from disjoint families.",
        ),
      );
    }
  ...
  return el(
    ...
    kvList(
      ...
      kv(
        "Agreement",
        markedValue(
          `κ = ${fmtStat(a.kappa)} · α = ${fmtStat(a.alpha)} · AC1 = ${fmtStat(a.ac1)}`,
          "calibrated",
        ),
        cite("gwet2014"),
        a.ci
          ? el(
              "span",
              { class: "faint data" },
              ` 95% CI [${fmtStat(a.ci.lo)}, ${fmtStat(a.ci.hi)}] · n = ${a.n}`,
            )
          : null,
      ),
      kv(
        ...
        cert.humanAgreement
          ? el(
              "span",
              { class: "data" },
              `κ = ${fmtStat(cert.humanAgreement.kappa)} · α = ${fmtStat(cert.humanAgreement.alpha)} (n = ${cert.humanAgreement.n})`,
            )
          : "—",
      ),
      ...
      kv(
        "Model pinned",
        cert.modelPinned ? "yes — snapshot recorded" : "no — stated plainly in methods",
      ),
      reliabilityHref
        ? kv(
            "All sources",
            el("a", { href: reliabilityHref }, "the construct's reliability matrix →"),
          )
        : null,
    ),
    a.perClass?.length
...
async function postStability(slug, instId, body) {
  ...
  try {
    res = await fetch(
      `/api/projects/${encodeURIComponent(slug)}/instruments/${encodeURIComponent(instId)}/stability`,
      {
        method: "POST",
  ...
  throw new ApiError("HTTP_" + res.status, `POST stability → ${res.status}`, {
    status: res.status,
  });
}
...
function actionRow(
  ...
  params,
  inst,
  { onPreviewed, previewScope = null, construct = null, catalog = {}, disposers = [] } = {},
) {
    ...
    "button",
    {
      ...
      onclick: async () => {
        ...
        clear(out).append(
          el(
            "p",
            { class: "faint", role: "status" },
            `${glyph.GLYPH} the Director is writing the prompt from the construct — ~30–60 s`,
          ),
        );
        try {
          const next = await api.instruments.compile(params.slug, inst.id);
          stop();
          toast.success(`Compiled v${next.version}.`, {
            detail: "Director-compiled — review the prompt below and edit to make it yours",
            data: false,
          });
          await refreshProject(params.slug).catch(() => {});
      ...
      onclick: () => {
        ...
        clear(out).append(
          el(
            "p",
            { class: "overline" },
            "Silver-tuning — Director labels a sample, the worker iterates",
          ),
        );
        ...
        silverHandle?.close?.();
        silverHandle = api.instruments.silverTune(
          params.slug,
          inst.id,
          { corpusId: previewScope?.corpusId ?? undefined },
          {
            ...
            onIteration(it) {
                  ...
                  el(
                    ...
                    prev !== null
                      ? el(
                          "span",
                          {
                            class: `itercard__delta ${it.agreement >= prev ? "" : "itercard__delta--down"}`,
                          },
                          ` ${it.agreement >= prev ? "+" : "−"}${fmtPct(Math.abs(it.agreement - prev), 0)}`,
                        )
                      : null,
                  ),
              ...
              if (!chart) {
                ...
                chart = line.render(
                  cWrap,
                  [{ label: "agreement", emphasis: true, points: [...pts] }],
                  {
                    caption: "agreement curve",
                    formatX: (x) => `it ${x}`,
                    formatY: (v) => fmtPct(v, 0),
                    dots: true,
                    height: 130,
            ...
            onDone(final) {
              ...
              out.append(
                el(
                  ...
                  final?.stability?.alpha !== undefined
                    ? ` · test–retest α = ${fmtStat(final.stability.alpha)}`
                    : "",
                  final?.level
                    ? el(
                        "span",
                        {},
                        " — ",
                        ladderC.render({ level: final.level, size: "sm", label: true }),
                      )
                    : null,
                  " ",
            ...
            onError(err) {
              ...
              stop();
              out.append(
                el("p", { class: "faint" }, "Tuning stream failed: ", String(err.message ?? err)),
              );
            },
          },
        );
  ...
  function stabilityPanel() {
    ...
    const paintCorpus = () => {
      ...
      clear(corpusLine).append(
        ...
        el(
          "span",
          { class: "data" },
          corpus ? scopechip.displayName(corpus) : "no corpus — import one first",
        ),
        corpus
          ? frag(
              " (text: ",
    ...
    const paintChips = () => {
          ...
          el(
            ...
            el(
              "button",
              {
                class: "termchip__x",
                type: "button",
                aria: { label: `Remove alternate judge ${m.model}` },
                onclick: () => {
                  altModels.splice(i, 1);
                  paintChips();
                  paintCalls();
                },
              },
      ...
      if (!altModels.length) {
        chipRow.append(
          el("span", { class: "faint" }, "none — the check reruns this instrument itself only"),
        );
      }
    };
      ...
      "button",
      {
        ...
        onclick: async () => {
          ...
          try {
            ...
            const res = m
              ? await postStability(params.slug, inst.id, {
                  k,
                  n,
                  corpusId,
                  models: altModels.map((x) => ({ ...x })),
                })
            ...
            if (res.pass && !inst.frozen && res.level === "stabilized") {
              toast.success("Stability passed — instrument is ◑.", {
                detail: `α = ${fmtStat(res.alpha)}`,
                data: true,
              });
            } else if (res.pass) {
              toast.success("Test–retest passed (α ≥ .80).", {
                detail: `α = ${fmtStat(res.alpha)}`,
                data: true,
              });
            }
            if ((res.alts ?? []).some((a) => a.error === undefined)) {
              toast.info(
                "Alternate judges labeled the same sample — model-vs-model κ/α is in Reliability →",
              );
            }
            await refreshProject(params.slug).catch(() => {});
          } catch (err) {
      ...
      el(
        ...
        el(
          ...
          el(
            "label",
            { class: "controlrow__item" },
            el("span", { class: "overline" }, "reruns k"),
            kInput,
          ),
          el(
            "label",
            { class: "controlrow__item" },
            el("span", { class: "overline" }, "units n"),
            nInput,
          ),
        ),
        isJudge
          ? el(
              ...
              el(
                ...
                `Also judge with (optional, up to ${STABILITY_MAX_ALTS})`,
              ),
              altPicker.el,
              chipRow,
              el(
  ...
  function paintStabilityResult(res, checkCorpusId = null) {
    ...
    if (!res.pass) {
      verdict = el(
        "span",
        {},
        "— below the .80 bar (Krippendorff's reliable threshold",
        cite("krippendorff2004"),
        "); the instrument changes its labels when rerun on the same units.",
      );
    } else if (promoted) {
      verdict = el(
        "span",
        {},
        "— the instrument gives the same labels when rerun on the same units. ",
        el("strong", {}, "Marked ◑ stabilized."),
      );
    } else if (inst.frozen) {
      verdict = el(
        "span",
      ...
      verdict = el(
        "span",
        {},
        "— Test–retest passed (α ≥ .80). Silver-tune + a passing stability check together mark ◑.",
      );
    }
    clear(out).append(
      el(
      ...
      onclick: async () => {
        ...
        clear(out).append(
          el(
            "p",
            { class: "faint", role: "status" },
            "previewing on 5 sample units (no run record or outputs persist; spend is metered)…",
          ),
        );
        try {
          const corpus = previewScope?.corpus() ?? null;
          ...
          if (units.length === 0)
            throw new Error(
              corpus
                ? `no units to preview on in ${scopechip.displayName(corpus)}`
                : "no corpus units to preview on",
            );
          // live envelope: {outputs, cost, quarantine, missing} — quarantine
          // entries are {unitId, code, message} (older servers: bare unit ids)
          const res = await api.instruments.preview(params.slug, inst.id, {
            unitIds: units.map((u) => u.id),
            corpusId: corpus?.id ?? undefined,
          });
              ...
              outputs.length
                ? el(
                    ...
                    el(
                      ...
                      el(
                        ...
                        el("th", { scope: "col", class: "table__num data" }, "conf"),
                        el("th", { scope: "col" }, "rationale"),
                      ),
                    ),
                    el(
                      "tbody",
                      ...
                      ...outputs.map((o) =>
                        el(
                          ...
                          el(
                            ...
                            el(
                              "button",
                              {
                                ...
                                type: "button",
                                dataset: { evidence: o.unitId },
                              },
                              String(o.unitId).slice(0, 8) + "…",
                            ),
                          ),
                          el(
                            "td",
                          ...
                          el(
                            "td",
                            { class: "table__num data" },
                            o.confidence !== undefined ? fmtStat(o.confidence) : "—",
                          ),
                ...
                : failed
                  ...
                  : el(
                      "p",
                      { class: "faint" },
                      "No outputs came back for these units — and none were quarantined; the server may not know these unit ids.",
                    ),
            ),
          );
          onPreviewed?.();
          ...
          clear(out).append(
            el("p", { class: "faint" }, "Preview failed: ", String(err.message ?? err)),
          );
        }
        stop();
      },
...
function freezeSheet(params, inst, previewScope = null, construct = null) {
  ...
  s.body.append(
    el(
      "p",
      {},
      "Freezing stamps the version hash into a calibration certificate with its agreement-vs-gold numbers. The instrument becomes read-only; any future edit forks a new ◌ version.",
    ),
    pickerHost,
  );
  ...
  api.goldsets
    ...
    .then((sets) => {
      ...
      select = el(
        "select",
        { class: "input", "aria-label": "Gold set" },
        el("option", { value: "" }, "choose a gold set…"),
        ...own.map((g) => el("option", { value: g.id }, `${goldName(g)} (${g.status})`)),
      );
      select.addEventListener("change", () => {
        goldsetId = select.value || null;
      });
      pickerHost.append(
        el(
          "label",
          { class: "field" },
          el("span", { class: "field__label overline" }, "Certify against"),
          select,
        ),
        el(
          "p",
          { class: "screen__hint faint" },
          createGoldBtn("or create another…", "btn btn--quiet"),
        ),
      );
    })
    .catch(() => {
      ...
      onclick: async () => {
        ...
        try {
          ...
          toast.success("Instrument frozen at ●.", {
            detail: `κ = ${fmtStat(cert?.agreement?.kappa)} vs gold · certificate written`,
            data: true,
          });
          await refreshProject(params.slug).catch(() => {});
          window.dispatchEvent(new HashChangeEvent("hashchange"));
...
const KIND_LINES = {
  judge:
    "A model given your codebook as instructions — the Director compiles the prompt from the construct.",
  dictionary:
    "A transparent term list — instant, free, fully auditable. You edit the terms; no model involved.",
  panel:
    "Several judges from disjoint model families that vote — catches any one model's blind spots.",
};
...
function compileSheet(params, { project, constructs, catalog, presetConstructId = null } = {}) {
  if (!constructs?.length) {
    toast.info("Write a construct first.", {
      detail: "instruments compile from the codebook — opening Constructs",
    });
  ...
  let constructId =
    presetConstructId && constructs.some((c) => c.id === presetConstructId)
      ? presetConstructId
      : constructs[0].id;
  let kind = "judge";

  /* construct picker */
  const constructSelect = el(
    "select",
    { class: "input", "aria-label": "Construct to measure" },
    ...constructs.map((c) => el("option", { value: c.id, selected: c.id === constructId }, c.name)),
  );
  constructSelect.addEventListener("change", () => {
    constructId = constructSelect.value;
  });

  /* model slot — defaults to the project Director's provider/model */
  const providers = Object.keys(catalog ?? {});
  let provider =
    project?.director?.provider && catalog?.[project.director.provider]?.length
      ? project.director.provider
      : providers[0];
  let model =
    (
      (catalog?.[provider] ?? []).find((m) => m.id === project?.director?.model) ??
      (catalog?.[provider] ?? [])[0]
    )?.id ?? null;
  const picker = modelpicker.render({
    catalog,
  ...
  const judgeOnly = el(
    ...
    el(
      "p",
      { class: "field__hint" },
      "Defaulted to this project's Director model — change it freely; the run preflight quotes the cost either way.",
    ),
  );

  const GO_LABELS = {
    judge: "Compile the judge",
    dictionary: "Create the dictionary",
    panel: "Create the panel",
  };
  const kindList = el(
    "div",
    { class: "choicelist", role: "radiogroup", aria: { label: "Instrument kind" } },
      ...
      el(
        ...
        el(
          ...
          el(
            ...
            k,
            k === "judge"
              ? el("span", { class: "chip chip--ghost choice__advised" }, "most studies start here")
              : null,
          ),
      ...
      onclick: async () => {
        ...
        try {
          if (kind === "judge") {
            ...
            inFlight = false;
            stop();
            toast.success(`Compiled “${name}” v${compiled.version ?? 2}.`, {
              detail: "review the prompt — edit it to make it yours",
              data: false,
            });
            await refreshProject(params.slug).catch(() => {});
            s.close();
