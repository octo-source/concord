...
export function disagreementsOf(goldset) {
  const coders = (goldset.coders ?? [])
    .map((c) => ({ rec: c, uncodable: uncodableSetOf(c) }))
    .filter(
      ({ rec, uncodable }) =>
        (rec.labels && Object.keys(rec.labels).length > 0) || uncodable.size > 0,
    );
  const out = [];
  ...
  asyncMount(
    ...
    async () => {
      ...
      const columns = goldset.corpusId
        ? await api.corpora
            .columns(params.slug, goldset.corpusId)
      ...
      return {
        project,
        goldset,
        construct: constructs.find((c) => c.id === goldset.constructId),
        columns,
      };
    },
    ({ project, goldset, construct, columns }) => {
      if (!state.pane) {
        state.pane =
          goldset.status === "sampling" || !goldset.sample?.length
            ? "sample"
            : goldset.status === "coding"
              ? "code"
              : goldset.status === "adjudicating"
                ? "adjudicate"
                : "test";
      }
      ...
      mount.append(
        contextLine([
          construct
            ...
            : {
                label: "gold for",
                text: goldset.constructId ?? "construct not recorded",
                faint: true,
              },
          {
            label: "sampled from",
            text: goldCorpus
              ? corpusText(goldCorpus, project)
              : goldset.corpusId
                ? `${goldset.corpusId} — corpus no longer in this project`
      ...
      const drawPane = () => {
        ...
        if (state.pane === "sample")
          samplePane(paneHost, params, goldset, construct, { columns, project });
      ...
      for (const pane of PANES) {
        ...
        const openCount =
          pane === "adjudicate" && disagreements.length
        ...
        tabs.append(
          el(
            "button",
            {
              ...
              aria: {
                label: openCount !== null ? `${PANE_LABELS[pane]} — ${openCount} open` : null,
              },
              onclick: () => {
                state.pane = pane;
                drawPane();
            ...
            openCount !== null
              ? el(
                  "span",
                  { class: "chip chip--signal data panetab__count", aria: { hidden: "true" } },
                  String(openCount),
                )
              : null,
          ),
...
function latestRunForConstruct(project, constructId) {
  const instIds = new Set(
    (project?.instruments ?? []).filter((i) => i.constructId === constructId).map((i) => i.id),
  );
...
function samplePane(host, params, goldset, construct, { columns = [], project = null } = {}) {
  ...
  const paintScale = () => {
    nOfPop.textContent =
      populationN !== null && populationN !== undefined
  ...
  if (goldset.sample?.length) {
    ...
    const piValues = [
      ...new Set(
        goldset.sample.map((s) => s.pi).filter((x) => typeof x === "number" && Number.isFinite(x)),
      ),
    ];
    const piChipText =
      piValues.length === 0
    ...
    host.append(
      section(
        ...
        el(
          "p",
          { class: "screen__hint" },
          el("span", { class: "data" }, fmtCount(goldset.sample.length)),
          ` units · design: ${goldset.design} · π stored per unit `,
        ...
        el(
          ...
          goldset.piNote ??
            "Inclusion probabilities are stored at sampling time — they are what make design-based correction (◉) possible later.",
        ),
      ),
    );
  }
  ...
  const peLine =
    source === "run"
      ? el(
          ...
          "Label shares from run ",
          el("span", { class: "data" }, runId ?? "—"),
  ...
  host.append(
    section(
      ...
      el(
        "ul",
        { class: "planrows", role: "list" },
        ...[100, 150, 300].map((size) =>
          el(
            "li",
            { class: "planrow" },
            ...
            el(
              "span",
              { class: "planrow__note faint" },
              size === 150
                ? "the usual gold-set size"
                : size === 300
                  ? "rare classes, many categories"
                  : "quick anchor",
            ),
          ),
        ),
      ...
      el(
        "p",
        { class: "screen__hint faint" },
        "Planning approximation (Cohen 1960 large-sample SE",
        cite("cohen1960"),
        cite("donner1992"),
  ...
  const designs = [
    { value: "srs", label: "Simple random", hint: "every unit equally likely — the cleanest π" },
    {
      value: "stratified",
      label: "Stratified",
      hint: `guarantees coverage across a metadata split (e.g., every ${strataExample} appears — n must be at least the number of groups); π varies by stratum, stored per unit`,
      needsColumns: true,
    },
    {
      value: "uncertainty",
      label: "Uncertainty",
      hint: "oversamples units the instrument is least sure about — efficient for finding failure modes; π still recorded. Not corpus-representative: π is recorded as nominal n/N over a deterministic ranking, so corrected estimates from this design are not design-unbiased. Use it for finding hard cases, not for correction.",
    },
  ];
  const strataSelect = el(
    "select",
    {
      class: "input input--inline",
      "aria-label": "Stratify by — the corpus's categorical columns",
    ...
    ...(categoricalCols.length
      ? categoricalCols.map((c, i) =>
          el(
            "option",
            { value: c.name, selected: i === 0 },
  ...
  host.append(
    section(
      ...
      el(
        "div",
        { class: "choicelist", role: "radiogroup", aria: { label: "Sampling design" } },
        ...designs.map((d) => {
          const blocked = d.needsColumns && !categoricalCols.length;
          return el(
            "label",
            { class: `choice${blocked ? " choice--disabled" : ""}` },
            el("input", {
              type: "radio",
              name: "design",
              value: d.value,
              checked: design === d.value,
              disabled: blocked,
              title: blocked
                ? "needs a categorical metadata column — none detected on this corpus"
                : null,
              onchange: () => {
                design = d.value;
                strataSelect.disabled = design !== "stratified" || !categoricalCols.length;
              },
            }),
            el(
              "span",
              ...
              el(
                "span",
                { class: "choice__hint" },
                blocked ? `${d.hint} — needs a categorical column; none detected` : d.hint,
              ),
            ),
          );
        }),
      ...
      el(
        ...
        el(
          "label",
          { class: "controlrow__item" },
          el("span", { class: "overline" }, "stratify by"),
          strataSelect,
        ),
      ),
      el(
  ...
  async function draw(force) {
    ...
    toast.success(`Sampled ${fmtCount(res.n ?? n)} units with π stored.`, {
      detail: `${res.design ?? design}${design === "stratified" && strata ? ` by ${strata}` : ""}`,
      data: true,
    });
  ...
  function confirmDiscardAndResample(err) {
    ...
    if (d) {
      if (d.labels > 0)
        parts.push(
          `${d.labels} unit${s_(d.labels)} handled (labels and can't-codes) by ${d.coders} coder${s_(d.coders)}`,
        );
    ...
    const sheet = openSheet({
      title: "Discard coded work and resample?",
      overline: "This gold set is already coded",
    });
    const goBtn = el(
      "button",
      {
      ...
      d?.labels > 0
        ? `Discard ${d.labels} handled unit${s_(d.labels)} and resample`
        : d?.adjudicated > 0
          ? `Discard ${d.adjudicated} adjudication${s_(d.adjudicated)} and resample`
    ...
    sheet.body.append(
      el(
        ...
        what
          ...
          : String(
              err.message ??
                "This gold set has committed coding work. Drawing a new sample discards it, including memos and flags.",
            ),
      ),
      el(
        "p",
    ...
    sheet.foot.append(
      el(
        "button",
        { class: "btn btn--quiet", type: "button", onclick: () => sheet.close() },
        "Keep labels",
      ),
      goBtn,
    );
...
function codePane(host, params, goldset, construct) {
  ...
  if (!total) {
    host.append(
      emptyState({
        title: "Nothing to code yet.",
        body: "Draw a sample first — the sprint starts the moment there are units to read.",
      }),
    );
    return;
  }
    ...
    section(
      ...
      coders.length
        ? el(
            ...
            ...coders.map((c) => {
              ...
              return el(
                ...
                el(
                  "span",
                  { class: "coderrow__bar", aria: { hidden: "true" } },
                  el("span", {
                    class: "coderrow__fill",
                    style: { width: `${Math.round(((done + cantCode) / total) * 100)}%` },
                  }),
                ),
                el("span", { class: "data" }, `${done}/${total}`),
                cantCode
                  ? el("span", { class: "chip chip--ghost data" }, `${cantCode} can't-code`)
                  : null,
                c.finishedAt ? el("span", { class: "chip chip--ghost" }, "complete") : null,
                c.flagged?.length
                  ? el("span", { class: "chip chip--signal data" }, `${c.flagged.length} flagged`)
                  : null,
              );
            }),
          )
        : el(
            "p",
            { class: "faint" },
            "No one has coded yet. The sprint reads units through the blind coder route — only unit text, the codebook, and your own progress reach this screen.",
          ),
    ),
  );
  ...
  const nameInput = el("input", {
    class: "input input--inline",
    placeholder: "coder id (e.g. sam)",
    "aria-label": "Coder id",
    value: continuingCoder(goldset) ?? "",
  });
  host.append(
    section(
      "Begin",
      el(
        ...
        el(
          "label",
          { class: "controlrow__item" },
          el("span", { class: "overline" }, "code as"),
          nameInput,
        ),
        el(
          "button",
      ...
      el(
        ...
        " memo · ",
        kbd("Esc"),
        " leave. The definition stays pinned.",
      ),
    ),
  );
}
...
function startSprint(params, goldset, construct, coder) {
  ...
  const root = el("div", {
    class: "sprint",
    role: "application",
    aria: { label: "Coding sprint" },
  });
  document.body.append(root);

  const progressLine = () =>
    `${codedCount} coded${cantCount ? ` · ${cantCount} can't-code` : ""} / ${total}`;
  const progressFill = el("span", {
    class: "sprint__progressfill",
    style: { width: `${((codedCount + cantCount) / total) * 100}%` },
  });
  const progressText = el(
    "span",
    { class: "sprint__progresstext data", aria: { live: "polite" } },
    progressLine(),
  );
  const paintProgress = () => {
    ...
    el(
      ...
      construct?.criteria?.exclude?.length
        ? el(
            ...
            { class: "sprint__defrule" },
            el("strong", {}, "Exclude: "),
            construct.criteria.exclude.join(" · "),
          )
        : null,
      ...
      construct?.examples?.length
        ? el(
            "details",
            { class: "sprint__defexamples" },
            el(
              "summary",
              { class: "sprint__defsummary" },
              `Worked examples (${construct.examples.length})`,
            ),
            ...construct.examples.map((ex) =>
              el("p", { class: "sprint__defrule" }, el("strong", {}, `${ex.label}: `), ex.text),
            ),
          )
        : null,
      el(
        ...
        { class: "sprint__defrule" },
        kbd("u"),
        " marks a unit as uncodable — it is excluded from agreement statistics and queued for adjudication.",
      ),
    ),
  );
  ...
  const keyRow = el(
    "div",
    { class: "sprint__keys", role: "toolbar", aria: { label: "Labels" } },
    ...
    el(
      "button",
      {
        class: "sprint__key sprint__key--meta",
        type: "button",
        dataset: { value: UNCODABLE },
        title:
          "Mark this unit uncodable — excluded from agreement statistics, queued for adjudication",
        onclick: () => uncodable(),
      },
      el("kbd", {}, "u"),
      el("span", { class: "sprint__keylabel" }, "can't code"),
    ),
    el(
      "button",
      { class: "sprint__key sprint__key--meta", type: "button", onclick: () => flag() },
      el("kbd", {}, "f"),
      el("span", { class: "sprint__keylabel" }, "flag"),
    ),
    el(
      "button",
      { class: "sprint__key sprint__key--meta", type: "button", onclick: () => memo() },
      el("kbd", {}, "m"),
      el("span", { class: "sprint__keylabel" }, "memo"),
    ),
  );
  ...
  root.append(
    el(
      ...
      el(
        "button",
        { class: "btn btn--quiet", type: "button", onclick: () => leave() },
        "Esc · leave",
      ),
    ),
  ...
  function drawUnit() {
    ...
    unitHost.append(
      el("blockquote", { class: "sprint__text" }, texts.get(id)?.text ?? "(unit text unavailable)"),
    );
  }
  ...
  async function label(value) {
    ...
    pulseKey(value);
    try {
      await api.goldsets.label(params.slug, goldset.id, {
        coder,
        unitId,
        label: value,
  ...
  function flag() {
    flagged = !flagged;
    toast.info(flagged ? "Flagged — saves with the next label." : "Flag cleared.", {
      duration: 1400,
    });
  }
  ...
  function memo() {
    ...
    pop.foot.append(
      el(
        "button",
        { class: "btn btn--quiet", type: "button", onclick: () => pop.close() },
        "Cancel",
      ),
      el(
        "button",
        {
          ...
          onclick: () => {
            memoText = ta.value;
            pop.close();
            toast.info("Memo holds — saves with the next label.", { duration: 1600 });
          },
        },
        "Keep memo",
      ),
  ...
  async function complete() {
    ...
    let fresh = goldset;
    try {
      fresh = await api.goldsets.get(params.slug, goldset.id);
    } catch {
      /* stale copy is still honest about status */
    }
    const codersWithLabels = (fresh.coders ?? []).filter(
      (c) => Object.keys(c.labels ?? {}).length > 0,
    );
    const headline =
      fresh.status === "complete" ? "Gold set complete" : "Your coding pass is complete";
    const done = el(
      "div",
      { class: "sprint__done" },
    ...
    if (codersWithLabels.length >= 2) {
      // agreement computes only when two coders' labels overlap
        ...
        el(
          "button",
          { class: "btn", type: "button", onclick: () => leave() },
          "Back to the studio",
        ),
      );
    }
...
function testPane(host, params, goldset, construct) {
  ...
  api.goldsets
    .agreement(params.slug, goldset.id)
    .then((report) => {
      clear(wrap);
      ...
      wrap.append(
        el(
          "p",
          { class: "screen__hint testpane__allsources" },
          el(
            "a",
            { href: `#/p/${params.slug}/reliability/${encodeURIComponent(goldset.constructId)}` },
            "all sources →",
          ),
          el(
            "span",
            { class: "faint" },
            " every reading of this construct — humans, gold, instruments — in one agreement matrix",
          ),
        ),
      );
      ...
      wrap.append(
        el(
          ...
          el(
            "p",
            { class: "overline humanbanner__label" },
            "Human–human agreement · computed before any machine comparison",
          ),
          el(
            "p",
            { class: "humanbanner__stat" },
            el("span", { class: "humanbanner__big data" }, `${kappaLabel} = ${fmtStat(h.kappa)}`),
            el("span", { class: "humanbanner__big data" }, `α = ${fmtStat(h.alpha)}`),
            el(
              "span",
              { class: "data faint" },
              `${fmtStat(h.percent)} raw agree · n = ${h.n ?? "—"} · ${(h.coders ?? []).join(" + ")}`,
            ),
          ),
          h.ci
            ? el(
                "p",
                { class: "data faint humanbanner__ci" },
          ...
          el(
            "p",
            { class: "humanbanner__note faint" },
            "Low human agreement is a construct problem before it is anyone's instrument problem.",
          ),
        ),
      );
          ...
          cols.append(
            el(
              ...
              el(
                "p",
                { class: "faint" },
                "Could not test against gold: ",
                el("span", { class: "data" }, inst.error.message ?? inst.error.code),
              ),
            ),
          );
          continue;
        ...
        const col = el(
          ...
          a.confusion
            ? el(
                "button",
                {
                  class: "btn btn--quiet btn--sm testcol__addreport",
                  type: "button",
      ...
      wrap.append(
        section(
          "Instruments against gold",
          cols,
          el(
            "p",
            { class: "screen__hint faint" },
            "AC1 is shown alongside κ and α because it stays stable under prevalence paradoxes — skewed label shares that sharply lower κ",
            cite("gwet2014"),
            ".",
          ),
          report.goldLabeled !== undefined
            ? el(
                "p",
                { class: "faint screen__hint data" },
                `${fmtCount(report.goldLabeled)} gold units (adjudicated, or ≥2 coders unanimous with no can't-code mark) backed this comparison`,
              )
            : null,
        ),
      );
    ...
    .catch((err) => {
      clear(wrap).append(
        emptyState({
          ...
          body: String(
            err?.message ??
              "Code the sample first — agreement computes the moment two coders overlap.",
          ),
        }),
      );
    });
...
function forestSection(wrap, report, { kappaLabel }) {
  const h = report.humanAgreement ?? {};
  const instr = (report.perInstrument ?? []).filter(
    (i) => !i.error && typeof i.agreement?.kappa === "number",
  );
  ...
  for (const inst of instr) {
    const a = inst.agreement;
    const ci =
      a.ci && typeof a.ci.lo === "number" && typeof a.ci.hi === "number"
        ? [a.ci.lo, a.ci.hi]
        : undefined;
    if (ci) anyCi = true;
    rows.push({
  ...
  forest.render(host, rows, {
    ...
    format: (v) => fmtStat(v),
    caption:
      `${kappaLabel} against gold per instrument — point is the coefficient, whisker the 95% CI` +
      (anyCi ? " (bootstrap percentile over the machine-vs-gold rows" : " (") +
      (report.goldLabeled !== undefined
        ? `, n = ${fmtCount(report.goldLabeled)} gold units)`
        : ")") +
...
export function benchmarkBand(value, ci, { stat = "α", legend = true } = {}) {
  ...
  const legendLine = kappaBands
    ? [
        "Landis & Koch's bands — κ .61–.80 substantial · .81–1.00 almost perfect",
        cite("landiskoch1977"),
        ". Context, never a gate.",
      ]
    : [
        "Krippendorff's working bands — α ≥ .800 reliable · .667–.800 tentative",
  ...
  return el(
    ...
    el(
      ...
      el("span", {
        class: "band__zone band__zone--mid",
        style: { left: pct(lo), width: pct(hi - lo) },
      }),
      el("span", {
        class: "band__zone band__zone--high",
        style: { left: pct(hi), width: pct(1 - hi) },
      }),
      ci
        ? el("span", {
            class: "band__ci",
            style: { left: pct(ci.lo), width: pct(Math.max(0.005, ci.hi - ci.lo)) },
          })
        : null,
...
function adjudicatePane(host, params, goldset, construct, disagreements = []) {
  ...
  const drawRow = (d) => {
    ...
    const row = el("div", {
      class: `adjrow${settledNow ? " adjrow--resolved" : ""}${d.excluded ? " adjrow--excluded" : ""}`,
    });
    const quoteHost = el("div", { class: "adjrow__quote" });
    // adjudication reads the same gold-set corpus the coders read
    api.evidence
      .get(params.slug, d.unitId, { corpusId: goldset.corpusId })
      .then((dossier) => {
    ...
    const decide = async (label) => {
      try {
        ...
        const res = await api.goldsets.adjudicate(params.slug, goldset.id, {
          unitId: d.unitId,
          label,
        });
        d.resolved = label;
        settle();
        row
          .querySelector(".adjrow__final")
          ?.replaceChildren(el("span", { class: "chip chip--gold" }, `final: ${label}`));
        toast.success("Adjudicated.", {
          detail: `${d.unitId} → ${label}${res?.status === "complete" ? " · gold set complete" : ""}`,
          data: true,
        });
      } catch (err) {
    ...
    const exclude = async () => {
      try {
        ...
        row
          .querySelector(".adjrow__final")
          ?.replaceChildren(el("span", { class: "chip chip--ghost" }, "excluded"));
        toast.success("Excluded from gold.", {
          detail: `${d.unitId} counts toward no agreement statistic and no gold label${res?.status === "complete" ? " · gold set complete" : ""}`,
          data: true,
        });
      } catch (err) {
      ...
      el(
        ...
        el(
          ...
          d.excluded
            ...
            : d.resolved
              ? el("span", { class: "chip chip--gold" }, `final: ${d.resolved}`)
              : null,
        ),
        !settledNow
          ? el(
              ...
              el(
                "button",
                {
                  ...
                  type: "button",
                  onclick: () => {
                    if (finalInput.value.trim()) decide(finalInput.value.trim());
                  },
                },
                "Set",
              ),
              el(
                "button",
                {
                  ...
                  type: "button",
                  title:
                    "Drop this unit from the gold set — it will count toward no agreement statistic and no gold label",
                  onclick: () => exclude(),
                },
                "Exclude from gold",
              ),
  ...
  if (construct?.categories?.length) {
    host.append(
      el(
        ...
        { class: "screen__hint faint" },
        "Valid labels: ",
        ...construct.categories.map((c) => el("span", { class: "chip" }, c.value)),
      ),
    );
  }
}
...
function openCoderSheet(params, goldset) {
  ...
  s.body.append(
    el(
      "p",
      {},
      "A coder session starts a second listener on this machine that serves a blind coding page for ",
      el("strong", {}, "one"),
      " coder on this gold set: the codebook, their next unit, and their own progress. Machine output, other coders' labels and adjudications are never on its API.",
    ),
    el(
      "div",
      { class: "controlrow" },
      ...
      el(
        "button",
        {
          ...
          onclick: async (e) => {
            ...
            e.target.disabled = true;
            clear(out).append(
              el("p", { class: "faint", role: "status" }, "starting the listener…"),
            );
            try {
              ...
              clear(out).append(
                el(
                  "p",
                  { class: "screen__hint" },
                  "The coder listener could not start (",
...
function confirmDeleteGoldset(params, goldset, err) {
  ...
  sheet.body.append(
    el(
      ...
      what
        ...
        : String(
            err.message ??
              "This gold set has committed coding work. Deleting it discards that work.",
          ),
    ),
    el(
      "p",
  ...
  sheet.foot.append(
    ...
    el(
      ...
      d?.labels > 0
        ? `Discard ${d.labels} handled unit${s_(d.labels)} and delete`
        : d?.adjudicated > 0
          ? `Discard ${d.adjudicated} adjudication${s_(d.adjudicated)} and delete`
