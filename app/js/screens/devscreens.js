...
const SCREENS = [
  ...
  ["Constructs", `#/p/${SLUG}/constructs/k_theme`, "pipeline strip with the compile handoff; structured editor; categories with explicit order; worked examples with kind chips; Director mark clears on first edit; draft/inductive sheets lock while a call runs (timer in the button, × = hide)"],
  ["Instruments — compile handoff", `#/p/${SLUG}/instruments?construct=k_burnout&compile=1`, "compile=1 + a construct with no instrument opens the New-instrument sheet directly: construct preselected, judge advised with one-liners, model defaulted to the project Director's"],
  ...
  ["Calibration Studio", `#/p/${SLUG}/goldsets/g_theme1`, "Sample (subset framing with live n-of-N, planning ±half-width as n changes, columns-driven stratify) · Code (full-bleed sprint: j/k, 1–9, f, m, timer, quiet completion) · Test (human-first banner + band, per-instrument nested agreement with confusion heat-tables, all-sources link) · Adjudicate (derived queue, pick-or-enter)"],
  ["Reliability home", `#/p/${SLUG}/reliability/k_theme`, "vs-gold headline rows with benchmark bands; sources×sources lower-triangle κ/α matrix (gold row gilded, ladder marks on instruments); cell → pair panel with disagreement-view doors; cited bands legend; teach-the-path partial states"],
  ["Reliability home (empty)", `#/p/${SLUG}/reliability/k_burnout`, "no readers yet: gold-sample CTA with price + compile door; no blank screens"],
