...
const KIND_HELP = {
  ...
  negative:
    "Looks relevant but does not qualify — the label carries the correct answer (e.g. absent, no).",
    ...
    (constructs) => {
      ...
      mount.append(
        screenHead({
          ...
          actions: [
            el(
              "button",
              { class: "btn", type: "button", onclick: () => newConstruct(params) },
              "+ New construct",
            ),
            el(
              "button",
              { class: "btn", type: "button", onclick: () => draftWithDirector(params) },
              `${glyph.GLYPH} Draft with Director`,
            ),
            el(
              "button",
              { class: "btn", type: "button", onclick: () => importCodebook(params) },
              "Import codebook",
            ),
            el(
              "button",
              { class: "btn btn--quiet", type: "button", onclick: () => inductiveMode(params) },
              "Inductive mode",
            ),
          ],
        }),
      ...
      if (!constructs.length) {
            ...
            actions: [
              el(
                "button",
                {
                  ...
                  type: "button",
                  onclick: () => draftWithDirector(params),
                },
                `${glyph.GLYPH} Draft with Director`,
              ),
              el(
        ...
        for (const k of constructs) {
          list.append(
            el(
              ...
              el(
                ...
                k.name,
                glyph.render({ authoredBy: k.authoredBy, humanTouched: k.humanTouched ?? true }),
              ),
              el(
                "span",
                ...
                k.categories?.length
                  ? el(
                      "span",
                      { class: "chip chip--ghost data" },
                      `${k.categories.length} categories`,
                    )
                  : null,
              ),
            ),
          ...
          const hasReadings =
            (project?.instruments ?? []).some((i) => i.constructId === k.id) ||
            ...
            list.append(
              el(
                "a",
                {
                  ...
                  title:
                    "Every reading of this construct — humans, gold, instruments — and how much they agree",
                },
                "reliability →",
              ),
            );
...
function editor(pane, params, construct, query = {}) {
  ...
  if (query.edgecase && !k.edgeCases?.some((e) => e.includes(query.edgecase))) {
    ...
    k.edgeCases.push(
      `From panel disagreement on ${query.edgecase}: (describe the boundary this unit exposes)`,
    );
    dirty = true;
  }
    ...
    "button",
    {
      ...
      onclick: async () => {
        ...
        try {
          ...
          const adopted =
            construct.authoredBy === "director" && !construct.humanTouched && k.humanTouched;
          toast.success("Construct saved.", {
            detail: adopted ? "Adopted — now yours to edit." : k.id,
            data: !adopted,
          });
          dirty = false;
  ...
  const hasInstrument = (store.get("project")?.instruments ?? []).some(
    (i) => i.constructId === k.id,
  );
  pane.append(
    pipeline.render({
      current: "construct",
      ...
      action: hasInstrument
        ? {
            label: "Open its instrument →",
            href: `#/p/${params.slug}/instruments?construct=${encodeURIComponent(k.id)}`,
          }
      ...
      el(
        ...
        el(
          "select",
          {
            ...
            "aria-label": "Construct type",
            onchange: (e) => {
              k.type = e.target.value;
              touch();
              redrawCategories();
              redrawExamples();
            },
    ...
    contextLine([
      k.draftedFrom
        ? draftedCorpus
          ? {
              label: "drafted from",
              text: scopechip.displayName(draftedCorpus),
              href: `#/p/${params.slug}/corpus/${draftedCorpus.id}/instant`,
            }
          : {
              label: "drafted from",
              text: `${k.draftedFrom} — corpus no longer in this project`,
              faint: true,
            }
        : null,
      measuringInstruments.length
        ? {
            ...
            node: el(
              "span",
              {},
              ...measuringInstruments.flatMap((inst, i) => [
                i ? ", " : null,
                el(
                  "a",
                  { class: "contextline__link", href: `#/p/${params.slug}/instruments/${inst.id}` },
                  inst.name,
                  " →",
                ),
              ]),
    ...
    section(
      ...
      el(
        "textarea",
        {
          ...
          value: k.definition ?? "",
          placeholder:
            "One or two sentences a stranger could apply — e.g. “The primary grievance the respondent treats as decisive. One label per unit: the deciding theme, not every theme mentioned.”",
          oninput: (e) => {
            k.definition = e.target.value;
            touch();
          },
        },
        k.definition ?? "",
      ...
      k.type === "ordinal"
        ? section(
            ...
            el(
              "p",
              { class: "screen__hint faint" },
              "This order feeds ordinal statistics (weighted κ, ordinal α) and sets the coding-sprint number keys.",
            ),
        ...
        : section(
            ...
            el(
              "p",
              { class: "screen__hint faint" },
              "Statistics treat these categories as unordered. The order here sets the coding-sprint number keys and the option order shown to model judges; for binary constructs with a dictionary instrument, the FIRST category is the positive label.",
            ),
            categoriesEditor(k, touch),
          ),
    );
  ...
  pane.append(
    section(
      ...
      editList("Rules for borderline cases, one per line", k.edgeCases, touch, {
        wide: true,
        placeholder:
          "e.g. “Mentions quitting hypothetically (‘if things don't change…’) → code as absent.”",
      }),
    ),
  );
      ...
      listEl.append(
        el(
          ...
          el(
            "button",
            {
              class: "btn btn--quiet editlist__remove",
              type: "button",
              aria: { label: "Remove" },
              onclick: () => {
                arr.splice(i, 1);
    ...
    k.categories.forEach((cat, i) => {
      wrap.append(
        el(
          ...
          el("input", {
            class: "input catrow__label",
            value: cat.label ?? cat.value,
          ...
          el(
            ...
            el(
              "button",
              {
                ...
                disabled: i === 0,
                onclick: () => {
                  [k.categories[i - 1], k.categories[i]] = [k.categories[i], k.categories[i - 1]];
                  touch();
                  redraw();
                },
              },
            ...
            el(
              "button",
              {
                ...
                disabled: i === k.categories.length - 1,
                onclick: () => {
                  [k.categories[i + 1], k.categories[i]] = [k.categories[i], k.categories[i + 1]];
                  touch();
                  redraw();
...
function examplesTable(k, touch) {
  ...
  const labelField = (ex, i, redraw) => {
    ...
    if (cats && k.type === "multilabel") {
      const current = Array.isArray(ex.label)
        ? ex.label
        : ex.label == null || ex.label === ""
          ? []
          : [ex.label];
      return el(
        "span",
        {
          class: "kindchips",
          role: "group",
          aria: { label: `Example ${i + 1} labels — every category that applies` },
        },
        ...cats.map((c) => {
    ...
    if (cats) {
      ...
      return el(
        "select",
        {
          class: "input extable__labelpick",
        ...
        ...(empty
          ? [el("option", { value: "", selected: true, disabled: true }, "pick its correct label")]
          : []),
        ...(!empty && !known
          ? [
              el(
                "option",
                { value: String(ex.label), selected: true },
                `${String(ex.label)} — not a current category`,
              ),
            ]
          : []),
        ...cats.map((c) =>
    ...
    if (k.type === "continuous") {
      return el("input", {
        ...
        value: ex.label ?? "",
        ...(k.scale != null
          ? { min: k.scale.min, max: k.scale.max, placeholder: `${k.scale.min}–${k.scale.max}` }
          : { placeholder: "number" }),
        "aria-label": `Example ${i + 1} label`,
        oninput: (e) => {
          ex.label = e.target.value === "" ? "" : Number(e.target.value);
          touch();
        },
      });
    }
    return el("input", {
      ...
      placeholder:
        k.type === "extraction"
          ? "the exact span(s) the coder should extract"
          : "the correct answer — e.g. yes",
      oninput: (e) => {
          ...
          el(
            ...
            el(
              ...
              el(
                "textarea",
                {
                  class: "input extable__input",
                  rows: 2,
                  "aria-label": `Example ${i + 1} text`,
                  placeholder:
                    "paste a real unit — e.g. “Base salary sat 18% under market and the refresh grants never came.”",
                  oninput: (e) => {
    ...
    wrap.append(
      ...
      el(
        "button",
        {
          ...
          type: "button",
          onclick: () => {
            k.examples.push({ text: "", label: "", kind: "positive" });
            touch();
            redraw();
          },
        },
...
function newConstruct(params) {
  ...
  s.body.append(
    el(
      "p",
      {},
      "Name the thing you want to measure and pick its answer type. The construct opens in the editor, where the definition, criteria, categories and worked examples are written.",
    ),
    el(
      "label",
      { class: "field" },
    ...
    el(
      "label",
      { class: "field" },
      el("span", { class: "field__label overline" }, "Type"),
      typeSel,
    ),
  );
  const createBtn = el(
    "button",
    {
      ...
      onclick: async (e) => {
        ...
        try {
          ...
          toast.success(`Construct “${created.name}” created.`, {
            detail: "write its definition and criteria, then save",
          });
          s.close();
...
function guardDirector(kind) {
  ...
  if (activeDirector.busy()) {
    if (activeDirector.sheetOpen()) activeDirector.focus();
      ...
      toast.info("The Director is still working on the last request.", {
        detail:
          "the proposals appear automatically when the Director finishes; you do not need to click again",
      });
    return true;
  }
  if (activeDirector.kind === kind) {
    // same sheet already open — refocus it
...
function draftWithDirector(params) {
  ...
  const s = openSheet({
    title: "Draft with the Director",
    overline: "Your concepts → draft constructs",
    onClose: () => {
      sheetOpen = false;
      stopBusy?.();
      if (inFlight) {
        toast.info("Hidden — the draft keeps running.", {
          detail:
            "the proposals sheet appears automatically when the Director finishes; you do not need to click again",
        });
  ...
  s.body.append(
    ...
    el(
      ...
      el("em", {}, "from the corpus"),
      " instead, use Inductive mode — that is hypothesis generation and is labeled as such.",
    ),
  );
    ...
    "button",
    {
      ...
      onclick: async () => {
        ...
        try {
          ...
          proposalsSheet(
            params,
            (res?.constructs ?? []).map((c) => ({ ...c, origin: "draft" })),
...
function inductiveMode(params) {
  ...
  const s = openSheet({
    title: "Inductive mode",
    overline: "Hypothesis generation — labeled as such",
    onClose: () => {
      sheetOpen = false;
      stopBusy?.();
      if (inFlight) {
  ...
  s.body.append(
    ...
    el(
      ...
      { class: "screen__hint" },
      "Inductive output is ",
      el("strong", {}, "hypothesis generation, not measurement"),
...
function proposalsSheet(params, proposals, titleLine, { note, sampleN } = {}) {
  ...
  const s = openSheet({
    ...
    onClose: () => {
      if (accepted > 0) repaintConstructs();
    },
  });
  if (sampleN) {
    s.body.append(
      el("p", { class: "faint screen__hint" }, `Read from a ${sampleN}-unit sample. `, note ?? ""),
    );
  }
  if (!proposals.length) {
    s.body.append(
      el(
        "p",
        { class: "faint" },
        "No proposals came back. Try more specific concepts, or check the Director model in Settings.",
      ),
    );
  } else {
    s.body.append(
  ...
  for (const prop of proposals) {
    ...
    const row = el(
      ...
      el(
        "div",
        { class: "proposal__text" },
        el(
          "h3",
          { class: "proposal__name" },
          prop.name,
        ...
        el(
          ...
          ...(prop.categories ?? []).map((c) =>
            el("span", { class: "chip" }, c.label ?? String(c.value)),
          ),
          ...(quoteRefs ?? [])
            .slice(0, 4)
            .map((id) =>
              el(
                "button",
                { class: "refchip data evidence-door", type: "button", dataset: { evidence: id } },
                String(id).slice(0, 8) + "…",
              ),
            ),
        ),
        prop.examples?.length
          ? quotecard.render({
              unit: { id: prop.examples[0].unitId ?? "example", text: prop.examples[0].text },
              compact: true,
            })
          : null,
      ),
      el(
        ...
        el(
          "button",
          {
            ...
            onclick: async (e) => {
              e.target.disabled = true;
              try {
                const created = await api.constructs.create(params.slug, {
                  ...constructBody,
                  authoredBy: "director",
                  humanTouched: false,
                });
                accepted += 1;
                toast.success(`Accepted “${created.name}” — it is in your codebook now.`, {
                  detail: "open it from the list to edit; editing marks it as yours",
                });
        ...
        el(
          "button",
          { class: "btn btn--quiet", type: "button", onclick: () => row.remove() },
          "Dismiss",
        ),
      ),
    );
