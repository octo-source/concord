...
export function render(mount, params) {
  ensureProject(params.slug).catch(() => {
    /* a missing project still allows the drop affordance */
  });
    ...
    destroy() {
      while (teardown.length) {
        ...
        try {
          ...
          console.error("import teardown threw", err);
        }
      }
    },
  };
...
function dropTarget(mount, params, teardown = []) {
  ...
  const zone = el(
    ...
    emptyState({
      ...
      actions: [el("label", { class: "btn btn--primary", for: "import-file" }, "Choose a file…")],
    }),
    input,
  );
  return zone;
...
function renderSheet(mount, params, proposal, file, teardown = []) {
  ...
  const meanLenOf = (col) =>
    col.stats?.meanLength ??
    Math.round(
      preview.reduce((s, row) => s + String(row?.[col.name] ?? "").length, 0) /
  ...
  mount.append(
    screenHead({
      ...
      lede: `Parsed locally — ${fmtCount(preview.length)} preview rows below.${
        tabular
  ...
  const confirmLabel = () =>
    tabular && unitTextColumn ? `Import — unit text from ${unitTextColumn}` : "Confirm import";
  if (tabular) {
    const choiceRow = (r) =>
      el(
        "label",
        ...
        el(
          ...
          el(
            ...
            r === textRanked[0]
              ? el("span", { class: "chip chip--ghost choice__advised" }, "longest text column")
              : null,
          ),
    ...
    mount.append(
      section(
        "Unit text — the column Concord measures",
        el(
          "p",
          { class: "screen__hint" },
        ...
        el(
          "div",
          { class: "choicelist", role: "radiogroup", aria: { label: "Unit text column" } },
          ...textRanked.map(choiceRow),
          textRanked.length === 0
            ? el(
                "p",
                { class: "screen__hint faint" },
                "No column was detected as text — pick one below.",
              )
            : null,
          others.length
            ? el(
                "details",
                { class: "utc-more", open: textRanked.length === 0 },
  ...
  /* -- column role chips -- */
  if (tabular) {
    const colList = el("div", { class: "colchips" }, ...columns.map((col) => columnChip(col)));
        ...
        el(
          ...
          ...proposal.issues.map((issue) =>
            el(
              ...
              el(
                "span",
                { class: "chip chip--signal issue__kind" },
                `${issue.kind ?? "issue"}${issue.count !== undefined ? ` · ${fmtCount(issue.count)}` : ""}`,
              ),
              el(
                "span",
                { class: "issue__note" },
                issue.note ?? issue.message ?? issue.detail ?? "",
              ),
            ),
          ),
        ),
    ...
    const options = [
      {
        scheme: "response",
        label: "Response",
        hint: "one unit per row — the natural unit for survey open-ends",
      },
      {
        scheme: "sentence",
        label: "Sentence",
        hint: "splits each row on sentence bounds — finer grain, more units",
      },
    ];
    mount.append(
      section(
        ...
        el(
          "p",
          { class: "screen__hint" },
          "Choose what one measured unit is. Junk and duplicates are scanned at confirm and import flagged, never dropped.",
        ),
        el(
          "div",
          {
            class: "choicelist choicelist--row",
            role: "radiogroup",
            aria: { label: "Unitization scheme" },
          },
          ...options.map((opt) =>
            el(
              "label",
              ...
              el(
                ...
                el(
                  ...
                  opt.label,
                  opt.scheme === "response"
                    ? el("span", { class: "chip chip--ghost choice__advised" }, "advised")
                    : null,
                ),
  ...
  mount.append(
    section(
      "Identifiers (emails, phones, SSNs, names)",
      el(
        "div",
        { class: "choicelist", role: "radiogroup", aria: { label: "Identifier handling" } },
        ...piiOptions.map((opt) =>
          el(
            "label",
            { class: "choice" },
    ...
    "button",
    {
      ...
      onclick: async () => {
        ...
        try {
          ...
          toast.success(
            `Corpus imported — ${fmtCount(result.unitCount)} units${textColumn ? ` from “${textColumn}”` : ""}.`,
            {
              detail:
                `${fmtCount(junkTotal)} flagged as junk (kept, marked)${unitization ? ` · ${unitization} unitization` : ""}` +
                ((result.skipped ?? 0) > 0
                  ? ` · ${fmtCount(result.skipped)} empty rows skipped`
                  : "") +
                (piiLine ? ` · ${piiLine}` : "") +
                (hadCorpora
                  ? " · Existing instruments can run on this corpus from Runs → New run."
                  : ""),
              data: true,
            },
          );
...
async function confirmImport(slug, body) {
  ...
  throw new ApiError("HTTP_" + res.status, `POST import/confirm → ${res.status}`, {
    status: res.status,
  });
}
...
function columnChip(col) {
  ...
  const wrap = el(
    "div",
    { class: "colchip", dataset: { role: col.role } },
    el(
      "div",
      { class: "colchip__head" },
      ...
      el(
        "span",
        { class: "colchip__conf", title: "Detection confidence" },
        el("span", {
          class: "colchip__confbar",
          style: { "--conf": `${Math.round((col.confidence ?? 0) * 100)}%` },
          aria: { hidden: "true" },
        }),
...
function progressRule(label) {
  ...
  const node = el(
    "div",
    { class: "progressrule", role: "status", aria: { label } },
