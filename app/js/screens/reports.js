    ...
    ({ project, methods, replication }) => {
      ...
      if (methods?.error) {
        ...
        const citations = new Map(
          (methods.citations ?? []).map((c) => [String(c.hash).slice(0, 8), c]),
        );
        methodsHost.append(
          el(
            "div",
          ...
          el(
              ...
              "button",
              {
                ...
                onclick: async (e) => {
                  ...
                  try {
                    ...
                    downloadText(
                      `${params.slug}-methods.md`,
                      record.markdown ?? "",
                      "text/markdown",
                    );
                    toast.success("Methods exported.", {
                      detail: `${params.slug}-methods.md — export.methods ledgered; every sentence keeps its citation`,
                      data: true,
                    });
                  } catch (err) {
            ...
            el(
              "span",
              { class: "faint data" },
              `${(methods.citations ?? []).length} ledger citations`,
            ),
          ),
        );
      }
      ...
      const goldToggle = () =>
        el(
          "label",
          { class: "switch repcard__toggle" },
          el("input", {
            type: "checkbox",
            checked: true,
            onchange: (e) => {
              goldState.includeGoldText = e.target.checked;
            },
          }),
          ...
          el(
            "span",
            { class: "faint" },
            " — gold rows include unit text by default; uncheck to ship labels/π only. License and PII review is yours.",
          ),
        );
      ...
      function downloadReplication() {
        ...
        const url =
          api.exports.replicationUrl(params.slug) +
          (goldState.includeGoldText ? "" : "?goldText=0");
        ...
        toast.success("Replication archive downloading.", {
          detail: goldState.includeGoldText
            ? "gold rows include unit text"
            : "goldText=0 — labels/π only, no unit text",
          data: true,
        });
      }
...
function citationChip(hash8, cite) {
  const chip = el(
    ...
    cite
      ? el(
          "span",
          { class: "citechip__pop", role: "tooltip", aria: { hidden: "true" } },
...
const BLOCK_TYPES = [
  ...
  {
    kind: "methods-excerpt",
    label: "Methods excerpt",
    hint: "the full generated methods document",
  },
];
...
function blockTitle(b) {
  ...
  if (b.kind === "quote")
    return `quote · ${String(b.ref ?? b.content?.attribution ?? "").slice(0, 16) || "verbatim"}`;
...
function reportCanvas(host, params, project) {
  ...
  const persist = async () => {
    ...
    try {
      ...
      if (cached?.slug === params.slug)
        cached.report = saved ?? { blocks, updatedAt: new Date().toISOString() };
    } catch (err) {
      ...
      try {
        ...
        store.set(
          "report.blocks",
          (fresh.report?.blocks ?? []).map((b) => ({ ...b })),
        );
      } catch {
        /* offline — keep the optimistic copy on screen */
      }
      redraw();
    }
  };
  ...
  host.append(
    ...
    el(
      "div",
      { class: "repcanvas__actions" },
      el(
        "button",
        {
          class: "btn",
          type: "button",
          onclick: () => addBlockSheet(params, project, redraw, persist),
        },
        "+ Add block",
      ),
      el(
...
function addBlockSheet(params, project, redraw, persist) {
  ...
  for (const bt of BLOCK_TYPES) {
    ...
    if (bt.kind === "chart" || bt.kind === "table") {
      const sel = el(
        "select",
        { class: "input input--inline", "aria-label": `${bt.label} source` },
          ...
          "button",
          {
            ...
            onclick: () => {
              ...
              push({
                ...
                title: `quote · ${input.value.trim().slice(0, 12)}`,
              });
            },
          },
          "Add",
...
async function renderReport(params, project) {
  ...
  if (typeof api.exports.replicationContents !== "function") {
    ...
    toast.success("Report streaming from the server.", {
      detail: `${params.slug}-report.html — full charts and evidence drill-down`,
      data: true,
    });
    return;
  }
  ...
  for (const b of blocks) {
    if (b.kind === "methods-excerpt") {
      ...
      parts.push(
        `<section class="block"><h2>Methods</h2><pre class="md">${escapeHtml(md)}</pre></section>`,
      );
      ...
      parts.push(
        `<section class="block"><blockquote class="quote">${escapeHtml(text)}</blockquote><p class="source">${escapeHtml(source)}</p></section>`,
      );
    } else if (b.kind === "text") {
      parts.push(`<section class="block"><p>${escapeHtml(b.content ?? "")}</p></section>`);
    } else {
      parts.push(
        `<section class="block"><h2>${escapeHtml(blockTitle(b))}${b.level ? ` <span class="mark">${markFor(b.level)}</span>` : ""}</h2><p class="note">Analysis ${escapeHtml(b.ref ?? "")} — full chart renders in the server export; this standalone draft records the reference and its evidence level.</p></section>`,
      );
    }
  }
...
${exploratory ? `<p class="watermark">EXPLORATORY ◌ — contains uncalibrated numbers; this watermark stays attached wherever these numbers are used.</p>` : ""}
  ...
  toast.success("Report rendered.", {
    detail: `${params.slug}-report.html${exploratory ? " — carries its ◌ watermark" : ""}`,
    data: true,
  });
}
