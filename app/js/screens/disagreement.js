  ...
  asyncMount(
    ...
    ({ data }) => {
      ...
      if (!data?.byEntropy?.length) {
        mount.append(
          emptyState({
            ...
            body:
              data?.note ??
              "No units cleared the entropy threshold. This can happen when the corpus is unambiguous or the jurors are too similar. Check the family chips on the panel.",
          }),
        );
        return;
      }
      ...
      heat.render(
        ...
        {
          caption:
            "Pairwise raw agreement between jurors. Low cells indicate a juror that agrees least with the others.",
          format: (v) => fmtStat(v),
        },
      );
      ...
      const queue = el("nav", {
        class: "split__list",
        aria: { label: "Disagreements by entropy" },
      });
      ...
      for (const item of data.byEntropy) {
        ...
        const btn = el(
          ...
          el(
            ...
            el(
              "span",
              { class: "entropy__track", aria: { hidden: "true" } },
              el("span", {
                class: "entropy__fill",
                style: { width: `${Math.round(item.entropy * 100)}%` },
              }),
            ),
      ...
      function drawDetail(item) {
        ...
        const facing = el("div", {
          class: "facing facing--n",
          style: { "--cols": String(Math.min(labels.length, 3)) },
        });
            ...
            el(
              ...
              line?.confidence !== undefined
                ? el(
                    ...
                    "conf ",
                    fmtStat(line.confidence),
                    el("span", {
                      class: "confbar",
                      aria: { hidden: "true" },
                      style: { "--conf": `${Math.round(line.confidence * 100)}%` },
                    }),
                  )
                : null,
            ),
        ...
        api.evidence
          .get(params.slug, item.unitId)
          .then((dossier) => {
            quoteHost.append(quotecard.render({ unit: dossier.unit, evidence: true }));
            const runOutputs =
              (dossier.outputs ?? []).find((o) => o.runId === params.rid)?.outputs ?? [];
            facing.replaceChildren(
              ...labels.map((l) =>
                judgeCol(
                  l,
                  runOutputs.find((o) => o.juror === l.juror),
                ),
              ),
            );
          })
      ...
      async function routeToHuman(item) {
        ...
        try {
          ...
          const g = goldsets[0];
          if (!g) {
            toast.warn("No gold set exists yet.", {
              detail: "create one in the Calibration Studio first",
            });
            return;
          }
          ...
          if (res?.already) {
            ...
            toast.success("Routed to the human queue.", {
              detail: `${item.unitId} → ${g.name ?? g.id} (queued with π = null — never a corrected-estimate row)`,
              data: true,
            });
          }
        } catch (err) {
      ...
      async function codebookDefect(item) {
        try {
          ...
          router.navigate(
            `p/${params.slug}/constructs/${k.id}?edgecase=${encodeURIComponent(item.unitId)}`,
          );
        } catch (err) {
