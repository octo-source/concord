...
function runGroups(params, project, runs) {
  ...
  const ordered = [...groups.entries()].sort(
    (a, b) => (a[0] === "__other__" ? 1 : 0) - (b[0] === "__other__" ? 1 : 0),
  );
  ...
  for (const [key, groupRuns] of ordered) {
    ...
    const head = el(
      "div",
      { class: "rungroup__head" },
      el(
        "h3",
        { class: "overline rungroup__title" },
      ...
      corpus
        ...
        : el(
            "p",
            { class: "scopechip faint" },
            "runs whose corpus is no longer in this project, or was never recorded",
          ),
    );
...
function runRow(params, project, r) {
  ...
  return el(
    "a",
    { class: "runrow", href: `#/p/${params.slug}/runs/${r.id}` },
    el(
      "span",
      { class: "runrow__name", title: r.id },
    ...
    el(
      "span",
      { class: "runrow__bar", aria: { hidden: "true" } },
...
async function preflightSheet(params, project, instruments, presetInstrument) {
  const s = openSheet({
    title: "Preflight",
    overline: "Cost and privacy check before starting",
    wide: true,
  });
  const corpora = project.corpora ?? [];
  ...
  const instSelect = el(
    "select",
    { class: "input", "aria-label": "Instrument" },
    ...instruments.map((i) =>
      el("option", { value: i.id, selected: i.id === instrumentId }, `${i.name} (${i.level})`),
    ),
  );
  instSelect.addEventListener("change", () => {
    instrumentId = instSelect.value;
    runPreflight();
  });
  ...
  const corpusSelect = el(
    ...
    {
      class: "input",
      "aria-label": "Corpus — picking a corpus picks the text column the instrument reads",
    },
    ...corpora.map((c) =>
    ...
    frag(
      el(
        ...
        el(
          ...
          el("span", { class: "overline" }, "instrument"),
          instSelect,
        ),
        el(
          "label",
          { class: "controlrow__item controlrow__item--grow" },
          el("span", { class: "overline" }, "corpus"),
          corpusSelect,
        ),
      ),
      corpora.length > 1
        ? el(
            ...
            "Same instrument, different data: pick the corpus to read.",
          )
        : null,
      scopeHost,
      resultHost,
  ...
  async function runPreflight() {
    ...
    try {
      ...
      const capInput = el("input", {
        ...
        onchange: (e) => {
          capUSD = e.target.value === "" ? null : Number(e.target.value);
        },
      });
        ...
        kvList(
          kv(
            ...
            el(
              "span",
              { class: "faint" },
              ` → ${fmtCount(pf.calls)} call${pf.calls === 1 ? "" : "s"}`,
            ),
          ),
          kv(
            "Tokens",
            el(
              "span",
              { class: "data" },
              `~${fmtCount(pf.inputTokens)} in · ~${fmtCount(pf.outputTokens)} out`,
            ),
          ),
          kv(
            "Estimated cost",
            el("span", { class: "data preflight__cost" }, fmtCost(pf.estUSD)),
            el(
              "span",
              { class: "faint" },
              " — assumes the full output-token budget; excludes Director second opinions and retries",
            ),
          ),
          ...
          kv(
            ...
            pf.privacyOk
              ? el(
                  "span",
                  { class: "preflight__privacy preflight__privacy--ok" },
                  "✓ allowed under this project's mode",
                )
              : el(
                  "span",
                  { class: "preflight__privacy preflight__privacy--blocked" },
                  "✕ blocked — ",
                  pf.privacyError ?? "this backend is not allowed under the project's privacy mode",
                ),
          ),
          kv(
            "Budget",
            hasCap
              ? el(
                  ...
                  pf.budget.wouldExceed
                    ? el("span", { class: "chip chip--signal" }, " would exceed")
                    : null,
                )
              : "no project cap",
          ),
          kv(
            ...
            el(
              "span",
              { class: "faint" },
              " USD — the run stops at the cap; resuming later continues without re-paying finished units",
            ),
          ),
        ),
      );
      ...
      clear(resultHost).append(
        el("p", { class: "faint" }, "Preflight failed: ", String(err.message ?? err)),
      );
    }
  }
  runPreflight();
...
function renderDetail(mount, params) {
  asyncMount(
    ...
    async () => {
      const project = await ensureProject(params.slug);
      const run = (project.runs ?? []).find((r) => r.id === params.id) ?? {
        id: params.id,
        status: "unknown",
      };
      return { project, run };
    ...
    ({ project, run }) => {
          ...
          "button",
          {
            ...
            onclick: async (e) => {
              ...
              try {
                ...
                toast.info(label === "Start" ? "Run started." : "Resumed.", {
                  detail: run.id,
                  data: true,
                });
                await refreshProject(params.slug).catch(() => {});
      ...
      const ledeFor = {
        ...
        paused:
          "Paused. Outputs already on disk are kept; resuming continues from the checkpoint without re-paying.",
        aborted:
          "Stopped. Outputs already on disk are kept; a resume continues from the checkpoint without re-paying.",
        complete:
          "Complete. Explore the results, or analyze them in the Workbench — every analysis reads one run's outputs.",
      ...
      const csvBtn = () =>
        el(
          "button",
          {
            ...
            type: "button",
            title:
              "Your rows back, plus the run's columns: the label under the construct's name, confidence when the model reported one (dictionary runs have none), escalated, an error column when units quarantined, and unit_id.",
          ...
          actions:
            run.status === "complete"
              ? [
                  el(
                    "a",
                    { class: "btn btn--primary", href: `#/p/${params.slug}/explore/${run.id}` },
                    "Explore results",
                  ),
                  csvBtn(),
                  el(
                    ...
                    {
                      class: "btn",
                      href: `#/p/${params.slug}/analyses?runId=${encodeURIComponent(run.id)}`,
                    },
                    "Analyze →",
                  ),
                  el(
                    "a",
                    { class: "btn", href: `#/p/${params.slug}/runs/${run.id}/disagreement` },
                    "Disagreement",
                  ),
                  el(
                    "a",
                    {
                      ...
                      href: run.instrumentId
                        ...
                        : `#/p/${params.slug}/runs`,
                      title:
                        "Start a new run of this instrument on a different corpus — pick the corpus in the preflight.",
                    },
        ...
        contextLine([
          instrument
            ? {
                ...
                href: `#/p/${params.slug}/instruments/${instrument.id}`,
              }
            : {
                label: "ran",
                text: run.instrumentId ?? "instrument not recorded",
                faint: !run.instrumentId,
              },
          { label: "on", text: corpusText(runCorpus, project, run.corpusId), faint: !runCorpus },
          runConstruct
            ? {
                label: "measures",
                text: runConstruct.name,
                href: `#/p/${params.slug}/constructs/${runConstruct.id}`,
              }
      ...
      if (run.error) {
        mount.append(
          el(
            "p",
            { class: "annotation annotation--still" },
            ...
            run.status === "failed" || run.status === "paused"
              ? el(
                  "span",
                  { class: "faint" },
                  " — Resume retries only the unfinished units; finished outputs are kept.",
                )
              : null,
          ),
        );
      ...
      const progFill = el("span", {
        class: "monitor__fill",
        style: { width: total0 ? `${(done0 / total0) * 100}%` : "0%" },
      });
      const progText = el(
        "span",
        { class: "data monitor__progresstext" },
        `${fmtCount(done0)} / ${fmtCount(total0)}`,
      );
      const costEl = el("span", { class: "monitor__cost data" }, fmtCost(run.cost?.actualUSD ?? 0));
      const escChip = el(
        "button",
        {
          class: "chip chip--signal monitor__esc",
          type: "button",
          onclick: () => escHost.scrollIntoView({ behavior: "smooth", block: "start" }),
        },
        `${run.escalation?.count ?? 0} escalations`,
      );
      const warnFeed = el("ul", {
        class: "monitor__warnings",
        role: "list",
        aria: { live: "polite" },
      });
        ...
        section(
          ...
          el(
            ...
            el(
              "div",
              { class: "monitor__row" },
              el("span", { class: "overline" }, "running cost"),
              costEl,
              el("span", { class: "overline" }, "escalations"),
              escChip,
              controlButtons(),
            ),
            el(
              "p",
              { class: "faint monitor__costnote" },
      ...
      const paintDist = (labelDist) => {
        ...
        if (!distChart) {
          clear(distHost);
          distChart = bar.render(distHost, data, {
            format: (v) => fmtStat(v),
            labelWidth: 120,
            valueWidth: 64,
          });
      ...
      if (run.status === "complete") {
        ...
        distHost.append(
          el("p", { class: "faint" }, "Partial — resume to continue; outputs so far are kept."),
        );
      } else if (!live) {
        distHost.append(
          el(
            "p",
            { class: "faint" },
            "Empty until the run starts — labels appear here as units finish.",
          ),
        );
      }
      ...
      const noWarningsLine = el(
        "li",
        { class: "monitor__warning monitor__warning--none faint" },
      ...
      const pushWarnings = (warnings = []) => {
        for (const w of warnings) {
          ...
          warnFeed.append(
            el(
              "li",
              { class: "monitor__warning" },
              el(
                "span",
                { class: "chip chip--signal" },
                typeof w === "object" && w?.kind ? w.kind : "watch",
              ),
              el("span", {}, text),
            ),
          );
      ...
      pushWarnings(
        run.error
          ? [
              {
                kind: String(run.error.code ?? "error"),
                message: run.error.message ?? "no message recorded",
              },
            ]
          : [],
      );
      ...
      if (live) {
        ...
        monitor = api.runs.monitor(params.slug, run.id, {
          onTick(t) {
            ...
            if (t.escalations !== undefined)
              escChip.textContent = `${t.escalations} escalation${t.escalations === 1 ? "" : "s"}`;
          ...
          async onDone(data) {
            ...
            if (status === "complete") {
              toast.success("Run complete.", {
                detail: `${run.id} — explore the results`,
                data: true,
              });
            } else if (status === "failed") {
              toast.error("Run failed.", {
                detail: `${run.id} — open it for the error; resume retries unfinished units`,
                data: true,
              });
            }
            ...
            const freshStatus =
              (fresh?.runs ?? []).find((r) => r.id === run.id)?.status ?? run.status;
            if (freshStatus === run.status) {
              ...
              pushWarnings([
                {
                  kind: "monitor",
                  message: `Stream ended (${status}) but the stored status is still ${run.status} — reload the page to reconnect.`,
                },
              ]);
              return;
            }
      ...
      api.runs
        .escalations(params.slug, run.id)
        .then((escalations) => {
            ...
            escHost.append(
              el(
                ...
                el(
                  "button",
                  {
                    class: "refchip data evidence-door escrow__unit",
                    type: "button",
                    ...
                    overridden
                      ...
                      : concurred
                        ? el(
                            ...
                            "worker verdict stands (Director concurred)",
                          )
                        : el(
                            "span",
                            { class: "chip chip--ghost" },
                            "flagged — no second opinion recorded",
                          ),
                  ),
                  el(
                    "p",
                    {},
                    el("span", { class: "chip chip--machine" }, String(esc.label)),
                    esc.confidence !== undefined
                      ? el("span", { class: "data faint" }, ` conf ${fmtStat(esc.confidence)}`)
                      : null,
                  ),
            ...
            el(
              ...
              q.unitId
                ? el(
                    "button",
                    {
                      class: "refchip data evidence-door quarrow__unit",
                      type: "button",
                      dataset: { evidence: q.unitId },
                    },
                    q.unitId,
                  )
                ...
                q.message ??
                  (q.code
                    ...
                    : el(
                        "span",
                        { class: "faint" },
                        "no reason recorded — this run predates quarantine reasons",
                      )),
              ),
            ),
          );
      ...
      mount.append(
        section(
          quarantined.length ? `Quarantined · ${fmtCount(quarantined.length)}` : "Quarantined",
          qHost,
        ),
      );
      ...
      mount.append(
        section(
          ...
          kvList(
            kv(
              "Instrument",
              el("span", { class: "data" }, run.instrumentId ?? "—"),
              " ",
              ladderC.render({ level, size: "sm" }),
            ),
            kv(
              "Model",
              el(
              ...
              run.pinned !== undefined
                ? el(
                    "span",
                    { class: "chip chip--ghost" },
                    run.pinned ? "pinned" : "unpinned — noted in the methods export",
                  )
                : null,
            ),
            kv(
              "Estimate",
              el("span", { class: "data" }, fmtCost(run.cost?.estUSD ?? 0)),
              el("span", { class: "faint" }, " preflight"),
            ),
          ...
          "button",
          {
            ...
            onclick: async () => {
              try {
                ...
                toast.error("Pause failed.", { detail: String(err.message ?? err) });
              }
            },
          },
          "Pause",
          ...
          "button",
          {
            ...
            onclick: async () => {
              try {
                ...
                toast.warn("Aborted.", {
                  detail:
                    "progress is saved — Resume continues from here without re-paying finished units",
                });
                await refreshProject(params.slug).catch(() => {});
                window.dispatchEvent(new HashChangeEvent("hashchange"));
              } catch (err) {
                toast.error("Abort failed.", { detail: String(err.message ?? err) });
              }
            },
          },
          "Abort",
