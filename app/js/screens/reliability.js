    ...
    if (!info) {
      byInst.set(
        ...
        (info = {
          ...
          name:
            String(s.label ?? "")
              .replace(/\s*—\s*rerun \d+ of \d+(\s*\(earlier version\))?$/, "$1")
              .trim() || instId,
          k: 0,
        }),
      );
...
export function render(mount, params, query = {}) {
  asyncMount(
    ...
    ({ project, construct, data, dataError }) => {
      ...
      const alphaHeadline =
        ordinal || ctype === "continuous" || ctype === "multilabel" || ctype === "extraction";
      const stat = alphaHeadline ? "α" : "κ";
      const statOf = (pair) =>
        alphaHeadline ? (pair.alpha ?? pair.kappa) : (pair.kappa ?? pair.alpha);
            ...
            node: el(
              ...
              construct
                ? el(
                    "a",
                    {
                      class: "contextline__link",
                      href: `#/p/${params.slug}/constructs/${encodeURIComponent(params.cid)}`,
                    },
                    constructName,
                    " →",
                  )
      ...
      const goldsetForConstruct =
        (project?.goldsets ?? []).find((g) => g.constructId === params.cid) ?? null;
      const firstInstrument =
        (project?.instruments ?? []).find((i) => i.constructId === params.cid) ?? null;
        ...
        mount.append(
          emptyState({
            ...
            actions: [
              ...
              el(
                "a",
                { class: "btn", href: compileHref(params, params.cid) },
              ...
              el(
                ...
                ...vsGold.map(({ source, pair }) => {
                  ...
                  return el(
                    ...
                    el(
                      ...
                      source.runId
                        ? el(
                            "a",
                            {
                              class: "vsgold__runlink data",
                              href: `#/p/${params.slug}/runs/${encodeURIComponent(source.runId)}/disagreement`,
                              title:
                                "This instrument's corpus run — open its per-unit disagreements",
                            },
                            source.runId,
                          )
                        : null,
                    ...
                    el(
                      "span",
                      { class: `vsgold__stat data vsgold__stat--${bandClass(v, stat)}` },
                      `${stat} ${fmtStat(v)}`,
                    ),
                    el(
                      "span",
                      { class: "vsgold__band" },
                      benchmarkBand(v, null, { stat, legend: false }),
                    ),
                    el(
                      "span",
                      { class: "vsgold__sub data faint" },
                      `${fmtPct(pair.percent, 0)} raw · n = ${fmtCount(pair.n)}`,
                    ),
                  );
                }),
              ),
      ...
      const panel = el(
        "aside",
        { class: "relpanel", aria: { label: "Pair detail", live: "polite" } },
        el(
          "p",
          { class: "relpanel__hint faint" },
          "Click any pair in the matrix to read its detail here.",
        ),
      );
      ...
      if (sources.length >= 2) {
        ...
        const replacesLine =
          retestLines.length || altHint
            ? el(
                "p",
                { class: "screen__hint faint" },
                "A newer stability check replaces these rows.",
              )
            : null;
        ...
        mount.append(
          section(
            ...
            altHint,
            replacesLine,
            el("div", { class: "relsplit" }, el("div", { class: "relsplit__main" }, matrix), panel),
            el(
              "p",
              { class: "relmatrix__legend faint" },
              "Reading the shades — κ .61–.80 “substantial”, .81–1.00 “almost perfect”",
              cite("landiskoch1977"),
              " · α ≥ .800 for confident conclusions, .667–.800 for tentative ones",
              cite("krippendorff2004"),
        ...
        mount.append(
          section(
            ...
            el(
              ...
              { class: "screen__hint" },
              "Only ",
              el("strong", {}, sources[0].label),
              " has read this construct so far — agreement needs a second reader. ",
              goldSource
                ? "Compile an instrument and run it, and the pair appears here."
                : "Code the gold sample, or add a second instrument.",
            ),
          ),
        );
      }
      ...
      if (instrumentSources.length === 1) {
        teach.push(
          el(
            ...
            el(
              "a",
              { class: "btn", href: compileHref(params, params.cid, true) },
              "+ Same construct, another model",
            ),
            el(
              "span",
              { class: "faint" },
              " a second instrument turns one reading into a reliability check — different model families catch different blind spots.",
            ),
          ),
        );
      }
      if (data?.retestAvailable === false) {
        teach.push(
          el(
            "p",
            { class: "relteach relteach--quiet faint" },
            "Test–retest: ",
            firstInstrument
              ? el(
                  "a",
                  {
                    href: `#/p/${params.slug}/instruments/${encodeURIComponent(firstInstrument.id)}`,
                  },
                  "run the Stability check on an instrument",
                )
              : el(
                  "a",
                  {
                    href: `#/p/${params.slug}/instruments?construct=${encodeURIComponent(params.cid)}`,
                  },
                  "run the Stability check on an instrument",
                ),
...
function buildMatrix({ params, sources, pairOf, stat, statOf, panel, goldsetForConstruct }) {
    ...
    frag(
      ...
      KIND_CHIP[s.kind]
        ? el(
            "span",
            {
              class: `chip ${s.kind === "retest" || s.kind === "alt" ? "chip--ghost" : "chip--gold"}`,
            },
            KIND_CHIP[s.kind],
          )
        : null,
  ...
  function cell(a, b) {
    ...
    if (!pair || pair.n === 0) {
      return el(
        ...
        el(
          "span",
          {
            class: "relcell relcell--empty faint",
            title: `${a.label} and ${b.label} never read the same units`,
          },
          "—",
        ),
      );
    ...
    const btn = el(
      "button",
      {
        ...
        onclick: () =>
          paintPair(panel, params, { a, b, pair, stat, value: v, goldsetForConstruct }),
      },
      el("span", { class: "relcell__value data" }, fmtStat(v)),
      el(
        "span",
        { class: "relcell__sub data" },
        `${fmtPct(pair.percent, 0)} · n ${fmtCount(pair.n)}`,
      ),
    );
  ...
  return el(
    "figure",
    { class: "relmatrix-wrap" },
    table,
    el(
      "figcaption",
      { class: "relmatrix__caption faint" },
...
function paintPair(panel, params, { a, b, pair, stat, value, goldsetForConstruct }) {
  const runLinks = [a, b]
    ...
    .map((s) =>
      el(
        "p",
        { class: "relpanel__link" },
        el(
          "a",
          { href: `#/p/${params.slug}/runs/${encodeURIComponent(s.runId)}/disagreement` },
  ...
  const goldLink =
    goldInPair && goldsetForConstruct
      ? el(
          "p",
          { class: "relpanel__link" },
          el(
            "a",
            {
              href: `#/p/${params.slug}/goldsets/${encodeURIComponent(goldsetForConstruct.id)}?pane=test`,
            },
  ...
  clear(panel).append(
    ...
    el(
      ...
      el(
        "div",
        { class: "kv" },
        el("dt", { class: "kv__label overline" }, "κ"),
        el("dd", { class: "kv__value data" }, fmtStat(pair.kappa)),
      ),
      el(
        "div",
        { class: "kv" },
        el("dt", { class: "kv__label overline" }, "α"),
        el("dd", { class: "kv__value data" }, fmtStat(pair.alpha)),
      ),
      el(
        "div",
        { class: "kv" },
        el("dt", { class: "kv__label overline" }, "raw agreement"),
        el("dd", { class: "kv__value data" }, fmtPct(pair.percent, 1)),
      ),
      el(
        "div",
        { class: "kv" },
        el("dt", { class: "kv__label overline" }, "jointly read"),
        el("dd", { class: "kv__value data" }, `${fmtCount(pair.n)} units`),
      ),
    ),
    ...
    runLinks.length || goldLink
      ? frag(
          ...
          el(
            "p",
            { class: "relpanel__note faint" },
            "Per-unit labels are on the linked disagreement pages; this panel shows the pair's statistics.",
          ),
        )
      : el(
          "p",
  ...
  for (const note of data?.notes ?? []) {
    mount.append(
      el(
        ...
        el("span", { class: "chip chip--ghost" }, "note"),
        " ",
        note,
      ),
    );
...
function createGoldBtn(params, project, corpusId, label, cls = "btn btn--primary") {
  ...
  if (existing) {
    return el(
      "a",
      { class: cls, href: `#/p/${params.slug}/goldsets/${encodeURIComponent(existing.id)}` },
