    ...
    ({ project, read }) => {
      if (!read || !read.lengthHist) {
        mount.append(
          emptyState({
            ...
            actions: [
              el("a", { class: "btn", href: `#/p/${params.slug}/import` }, "Back to import"),
            ],
          }),
        );
        return;
      ...
      mount.append(
        screenHead({
          ...
          lede:
            "Local counts only — lengths, languages, distinctive terms, metadata." +
          ...
          actions: [
            el(
              "span",
              {
                class: "chip chip--ghost localbadge",
                title: "Computed from bundled lexicons and local statistics",
              },
              "⌂ statistics computed locally — no model reads your data here",
              read.unitCount
                ? el("span", { class: "data" }, ` · ${fmtCount(read.unitCount)} units`)
                : null,
            ),
          ],
        }),
      ...
      mount.append(
        el(
          ...
          el(
            "div",
            { class: "unittext__line" },
            el(
              "p",
              { class: "unittext__statement" },
            ...
            el(
              "button",
              {
                class: "btn unittext__change",
                type: "button",
                title:
                  "Pick a different column as the unit text — builds a new corpus; this one is kept",
      ...
      const termList = el(
        "ol",
        { class: "termcols", role: "list" },
        ...(read.topTerms ?? []).map((t) =>
          el(
            "li",
            { class: "termcols__item" },
      ...
      grid.append(
        wrapCell(
          ...
          el(
            ...
            "Frequency damped by document frequency, stopwords removed. These are raw counts, not an interpretation.",
          ),
        ),
      );
      ...
      const priceTag = est
        ? isMock
          ? `${fmtCost(0)} · Mock`
          : `~${fmtCost(est.usd)}, ${fmtDuration(est.etaMin)}`
        : null;
      mount.append(
        el(
          "div",
    ...
    "button",
    {
      ...
      onclick: async () => {
        ...
        try {
          ...
          toast.success(
            `Re-unitized — ${fmtCount(res.unitCount)} units now read from “${res.textColumn}”.`,
            {
              detail:
                `${fmtCount(res.skipped ?? 0)} rows skipped (empty in that column) · the original corpus is kept` +
                (piiLine ? ` · ${piiLine}` : ""),
              data: true,
            },
          );
  ...
  (async () => {
    ...
    try {
      ...
      clear(listHost).append(
        el("p", { class: "faint" }, "Could not read sample units: ", String(err.message ?? err)),
      );
      return;
    }
    clear(listHost);
