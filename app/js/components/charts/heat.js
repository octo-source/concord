...
/* ---- pure geometry ------------------------------------------------------- */
export function layoutHeat(
  { rows = [], cols = [], values = [] },
  { width = 420, labelWidth = 96, colLabelHeight = 64, cellGap = 2, maxCell = 56 } = {},
) {
  ...
  return {
    ...
    max,
    cells,
    rowLabels: rows.map((label, r) => ({
      label,
      x: labelWidth - 8,
      y: colLabelHeight + r * (cell + cellGap) + cell / 2,
    })),
    colLabels: cols.map((label, c) => ({
      label,
      x: labelWidth + c * (cell + cellGap) + cell / 2,
      y: colLabelHeight - 8,
    })),
  };
}
...
export function toTable(
  { rows = [], cols = [], values = [] },
  { caption = "Heat map data", format = (v) => fmt(v, 0) } = {},
) {
  return dataTable(
    caption,
    ["", ...cols],
...
export function render(container, data, opts = {}) {
  const { caption = null, format = (v) => fmt(v, 0), showValues = true } = opts;
  ...
  return {
    ...
    update(next) {
      ...
      figure
        .querySelector(".chart__data table")
