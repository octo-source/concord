...
export function render(container, points, opts = {}) {
  ...
  const { figure, mount } = chartFigure({
    caption,
    table: toTable(points, {
      caption: caption ?? "Triangulation data",
      xLabel,
      yLabel,
      format,
      threshold,
  ...
  function draw(p, width, first) {
    ...
    for (const t of g.xTicks) {
      axis.append(
        svgEl(
          "text",
          { x: t.x, y: g.baselineY + 14, class: "chart__tick", "text-anchor": "middle" },
          format(t.value),
        ),
      );
    }
    ...
    axis.append(
      ...
      svgEl(
        ...
        {
          x: 12,
          y: (g.y.range[0] + g.y.range[1]) / 2,
          class: "chart__axislabel",
          "text-anchor": "middle",
          transform: `rotate(-90 12 ${(g.y.range[0] + g.y.range[1]) / 2})`,
        },
        yLabel,
      ),
    );
  ...
  return {
    ...
    update(next) {
      ...
      figure.querySelector(".chart__data table")?.replaceWith(
        toTable(current, {
          caption: caption ?? "Triangulation data",
          xLabel,
          yLabel,
