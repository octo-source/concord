...
**ingest:** `pii.pseudonymize` accumulates into an existing vault (idempotent; `VAULT_CONFLICT` on remap). `pii.scan/pseudonymize/reidentify` cover STRING metadata values exactly like unit text (same vault, same `[KIND_n]` tokens; non-string values pass through, types preserved); scan findings gain `meta?: [{column, spans}]`, unit `flags.pii` unions text+meta kinds. Transcript same-speaker merge gap ≤ 30s (`{maxMergeGapSeconds}` overrides).
...
Contracts: every parser → `{rows?: object[], docs?: {name, paras: string[]}[], turns?: {speaker, t0, t1, text}[]}`. `mapping.detect(rows)` → `{columns: [{name, role: "text"|"categorical"|"numeric"|"date"|"id"|"ignore", confidence, stats}]}` (text = mean length > 40 chars or >20 distinct long values; date = >80% parseable). `unitize(corpusId, parsed, scheme)` → `Unit[]` for schemes `response|sentence|paragraph|turn`. `junk.scan(units)` → flags: `na` (n/a, none, asdf row), `short`, `dup` (exact + whitespace-normalized), `bot` (≥3 identical non-trivial texts). `pii.scan(units)` → spans for emails/phones/SSN-like/names-heuristic, over unit text AND string metadata values (meta findings carry the column name); `pii.pseudonymize(units, vaultPath)` reversible, masks metadata values into the same vault. **Amended at wiring:** the vault lives IN the bundle at `projects/<slug>/vault/<corpusId>.json` but is excluded from the replication archive (the archive builds from an explicit member allowlist; the exclusion is test-pinned in `tests/server/pii-import.test.js`) — the "never leaves this machine" property is held by exclusion, not location.
